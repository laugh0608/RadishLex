...
use radishlex_ime_crypto::{
    CryptoError, DevicePrivateKeyStoreStatus, DeviceSignature, DeviceSigningKeyHandle,
...
impl SyncTrustedDeviceProfile {
    pub fn active(
        ...
        Self::new(device, signing_public_key, None)
    }
    ...
    B: SyncDeviceSigningBackend,
{
    fn freeze_cycle(
        ...
        let epoch_materials = self
            .epoch_materials
            .load_epoch_materials(domain_id, &self.local_device_id)
            .map_err(map_load_error)?;
...
fn map_load_error(error: SyncCryptoLoadError) -> SyncOrchestrationError {
    match error {
        SyncCryptoLoadError::Unavailable => backend_preflight(),
