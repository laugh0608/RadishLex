...
- production 源码、L6 format v1、acceptance controller、release-pair 与 maintenance-only refresh v1 合同已闭合。六类 operation 均有独立证据。首个 `install_prepared` checkpoint 暴露的 Debian `01777` startup 缺陷已由 `a6622d4` 修复，target `d75818f` 的 pair record `c74fac12…9849` 已冻结。两台新pair重试clone均在transaction前失败关闭：`45D8205F…F2B4`原因为input harness inventory/path不一致；`0BB24317…307C`的case把fresh absent错误预期为remove-terminal `RemovedProgram`，实际正确返回`ReceiptMissing`。后者manifest `66212f2c…a8a3e`证明无mutation preflight、operation ID、state、checkpoint或guard；关机freeze `65157b95…cb2b`固定磁盘与零句柄，二十一台VM现全部stopped。v1 package不含RadishLex maintainer scripts。
