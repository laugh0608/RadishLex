...
- production 源码、L6 format v1、acceptance controller、release-pair 与 maintenance-only refresh v1 合同已闭合。六类 operation 均有独立证据。首个 `install_prepared` checkpoint 暴露的 Debian `01777` startup 缺陷已由 `a6622d4` 修复，target `d75818f` 的 pair record `c74fac12…9849` 已冻结。两台新pair retry均在transaction前因harness缺陷失败关闭，持久证据manifest为`ae4d1da2…ab8d`/`d99cbfad…324c`。它们连同旧network失败与rollback transfer失败clone已按授权由plain `utmctl delete`移除注册项和package；cleanup manifest `e1930d6c…41f7c`证明未触及其余注册项，当前十七台VM全部stopped。v1 package不含RadishLex maintainer scripts。
