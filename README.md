...
`check-manager-ffi-smoke.sh` 会构建 `radishlex-ime-ffi` 动态库，并用临时 SQLite userdb 与合成词库文件复验 Dart FFI bridge 的本地 list / delete / import / export、learning status 和 sync preflight 摘要；该命令不连接真实同步后端，不读取真实输入法目录。
