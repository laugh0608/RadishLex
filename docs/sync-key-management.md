...
进入用户可用同步前，应按生产部署 runbook 补目标部署运行证据；Apple 原生非导出 Ed25519 支持矩阵应单独调查，Android Keystore 已补仓库内 Kotlin bridge source，但仍应补 JNI / Android instrumented smoke 与真实 API / 设备矩阵证据。access token 已有首个 server / transport 证据，但可用平台私钥 backend 停止线解除前，不应开放用户可用同步主线。
...
12. 已补 `android-keystore-v1` 平台 runbook、`android-keystore` feature、不可用状态门禁、Rust bridge wrapper、bridge contract、合成 bridge 单测、ignored smoke 入口和仓库内 Kotlin bridge source，固定 Android Keystore Ed25519 创建 / 加载 / 签名 / 删除、锁屏 / 权限、备份迁移、IME 生命周期和日志脱敏验证边界；当前未接 JNI / Gradle 或真实 Android Keystore smoke。
