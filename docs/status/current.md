...
- 复核：2026-09-09（已授权的 macOS 39→40 升级暂停：双程序为 40，外层 `data_coordinating`、数据 `candidate_verified`，原库未切换且 WAL/SHM 保留；Installer 未显示执行错误，现场冻结待修复/恢复方案）；常态分支 `dev`，主线 `master`。
...
- 按项目所有者批准，`6a55782` 改用 IMK 输入客户端身份；合同/native/全仓验证通过。build 40 最终 Installer、唯一受控 source 39 及包内 smoke 已完成；随后获准的正常升级 `aad9cf8a…a706` 已替换双程序，但在数据 `candidate_verified` 后继续未推进，外层保持 `data_coordinating`。原数据库 inode 与 WAL/SHM 保留，源程序备份仍为原 build 39；新版本启动和全部定向输入未执行。切换前 sidecar 拒绝条件与 bridge 丢弃执行错误的代码路径已定位，具体生产错误值未被暴露，不能把 `error=none` 视为通过。下一步先明确仓库修复与冻结现场恢复方案，详见[联合验收入口](../runbooks/macos-rev01-rev02-acceptance.md#build-3940-实机升级暂停2026-09-09)；REV-01/REV-02 继续开放。
