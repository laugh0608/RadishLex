...
Wayland 人工验收已覆盖 GNOME Text Editor、GNOME Terminal、Firefox 与 Qt6/FeatherPad 的 composition、框架候选、光标、翻页、方向/数字/Space/鼠标选择、commit、cancel/reset、应用/输入法切换、Fcitx 重启与 context 重连。候选专项已配对消费 addon 接受的 press/release；preedit 专项已设置 UTF-8 字节 cursor 和 Fcitx `DontCommit`，避免失焦时提交原始拼音。两项修复均通过 guest 强门禁和用户实体操作复验。unknown、terminal 和真实 password field 后的 userdb v9 学习聚合均为零；password 路由只使用 `keyboard-us` 且无候选。仅允许 `AF_UNIX/AF_NETLINK` 的进程级离线态仍可输入，随后已恢复普通进程。Electron、Sensitive capability、桌面会话重启、整台 guest 断网与 X11 仍待验证；详细版本和流水见本周周志。
...
3. M5-P03 已完成用户级开发装配、GTK/终端/Firefox/Qt6 Wayland 输入、完整候选交互、cancel/reset、焦点/输入法切换、Fcitx 重启、unknown/password 和进程级离线证据，并修复 press/release 与 preedit 失焦/cursor 生命周期。下一步先用官方 Electron ARM64 运行时完成本地离线应用验收，再验证 Sensitive capability、桌面会话重启和整台 guest 断网；Wayland 剩余矩阵稳定后进入 X11 兼容路径。
