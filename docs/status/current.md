...
- L6 format/controller、release-pair与maintenance refresh已完成，六类operation均有独立证据。首个crash已闭合；`install_artifacts_staged`旧clone两次start、v2 clone及新v3 clean clone的首次start均在host失败关闭，未进入guest。repository-only start-once、clone-once与host launch只读诊断控制均已闭合；二十台VM全部stopped，其余七个crash实机case和连续完整L6未闭合。
...
- repository-only `l6_utm_launch_diagnostics.py`现把后续只读host诊断绑定到clean committed head、executed-control identity及上述start/failure/postverify三份manifest；语义读取的`request.json`、`terminal.json`、`failure.evidence.txt`与`postverify.evidence.txt`必须实际列入对应manifest，且冻结终态必须仍为一次start、全停、无guest/input/operation/transaction。通过前置后唯一命令面为`utmctl list`、目标`utmctl status`、固定`/bin/ps`与最长15分钟的固定predicate `/usr/bin/log show`；只持久化有界进程身份、日志元数据、完整hash及脱敏前缀，terminal始终保留`root_cause=unattributed`。八项fake-runner/临时目录回归已接入默认L6门禁；尚未读取真实system log或生成真实诊断证据。
...
- host launch诊断的仓库控制闭合不构成读取macOS unified log或运行host观察命令的授权；真实采集必须限定冻结时间窗、目标、三份manifest与create-new输出根，并单独授权。日志事件只能作为观察量，不能凭单条文本归因或放宽v3冻结线。
...
1. 下一项系统动作须单独授权一次真实只读host诊断：只针对冻结v3 `5B19AEF1…7DAB`，绑定clean committed head与start/failure/postverify manifest `870f56dd…f3a5`/`59c62c14…bba05`/`8e3d5ced…8386`，在明确的历史启动时间窗内依次执行`utmctl list`、目标`utmctl status`、固定`/bin/ps`和固定predicate `/usr/bin/log show`，写入absent私有证据根。该批不含start、stop、clone、guest exec/file、input、operation ID、transaction、retry或delete。
2. 诊断结果只记录可验证的UTM/utmctl/QEMU进程、事件和全停状态；即使存在错误文本，缺少独立事实交叉证明时仍保持`root_cause=unattributed`，不预设bookmark、config、磁盘或权限根因，也不得复用或第二次启动v3。
...
上述入口以合成执行器证明单次start/clone、只读host launch诊断、各自确定与失败关闭terminal，以及零越界自动动作。新v3 clone的真实创建、磁盘冻结与单次start失败关闭已有独立host evidence，始终未进入guest；真实system log尚未读取，八case整体、连续完整L6与发布未闭合。
