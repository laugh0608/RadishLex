...
- L6 format v1、acceptance controller、release-pair 与 maintenance-only refresh v1 已完成。第五套为`rolled_back`，第六套upgrade为target `completed`；旧repair的staged verifier缺口已修复。新clone唯一refresh ELF调用虽形成`repair/same_release/completed` receipt，但健康产品短路使dpkg完全未运行，因此不满足repair重装合同；证据已冻结，十二台VM全停。
...
4. 下一步先修复`drive_target`对健康repair的提前成功短路，补“首次repair必须调用一次apply、已有target proof的crash retry不得重复”直接回归；闭合门禁后再形成新refresh handoff和S3 clone。其余矩阵继续关闭。
...
上述入口证明 prior-terminal anchor、target-only builder、canonical revision、single-target repair effective-source与production-only refresh合同。真实新ELF调用进一步暴露健康repair会在dpkg前提前成功，现有测试仅以invalid product强制进入apply。该语义修复、真实repair、其后主序列、八个crash case、桌面启动、重启、完整L6与公开发布仍未闭合。
