...
3. R01B 自动化代码批与实机前置门禁已经完成；`build 33` 的 contract、ad-hoc native 与仓库门禁已在不安装、不提供 Apple Development identity 的前提下通过。当前仍是未提交的已验证候选；下一步先提交本批源码与证据，再在 clean HEAD 使用同一版本化工具链和逐字节一致的隔离输入重跑门禁、复核哈希并完成冻结。
