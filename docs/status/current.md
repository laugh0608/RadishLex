...
M4-P02 已完成升级协调器边界、`ime-userdb` 只读 inspection/隔离 migration contract、SQLite backup API 一致快照，以及 `ime-product-upgrade` 单向状态机、receipt v1、固定数据根身份、原子持久化、跨进程 socket guard、保守空间预算、snapshot 与 migration candidate 编排和故障注入。当前实现能把未 checkpoint WAL 和 schema 0 空库复制为单文件隔离 snapshot，只在固定 candidate 上迁移并收敛为无 sidecar 的 `DELETE` journal 单文件，按 artifact identity 先固化证据再推进状态；原库和 snapshot 均不被 migration 改写。未知对象、symlink/hardlink、身份漂移、损坏 snapshot、非法状态替换和中断现场会失败关闭。真实 macOS available-space/静止证明、settings 副本、双端产品 validation host、原子切换与完整崩溃恢复尚未闭合，因此 M4-P02 未退出。
...
1. M4-P02 下一切面接入固定 macOS available-space/静止证明与 settings 副本，并实现 Manager/InputMethod 两个独立候选 validation host；随后进入原子切换与崩溃恢复。现有 `ValidationHost` 只证明 unknown/P0 上下文分类，不能冒充数据升级产品宿主；原库继续只读，当前没有 App Group 迁移需求，不得静默改变容器。
