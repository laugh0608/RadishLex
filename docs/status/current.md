...
- 当前主批次：M5-P04 Linux Manager、同库个人化与本地管理验收（已完成，停在 M5-P05 前）
...
同一修复 addon 随后完成 Manager 删除、既有 session 防复活、Fcitx 重启后新 session 防复活、explicit restore 与恢复后新 session 重新学习。删除期间普通选择只增加 P1 selection，不恢复 term/ranker 或改写 tombstone；恢复版本严格晚于删除版本，旧 frequency 不复活，首次新选择从 frequency 1 重新开始。该子链终态为 user terms 1、selection events 5、suppressed 0、deleted 0，多次 Fcitx 重启与 Manager 并发期间无投影、busy 或 migration 错误。

真实 Manager I/O 在同一 guest/XDG userdb 上以两条合成词闭合。dry run 显示新增 2 且数据库不变；普通导入后删除“爸”，再次导入只更新“巴”并跳过 tombstone。导出只含 active P2 词条“巴”和“把”，不含已删除词、P1 明细、session 或 context。

Manager 进程与完整 X11 桌面会话重启后，新 Manager/Fcitx 进程继续读取固定 bundle、同版 FFI 和相同数据库；候选仍为“巴”第一、“把”第二，Escape 保持 selection 5。最终聚合为 user terms 2、selection events 5、suppressed 0、deleted 1、import batches 2；schema v9/WAL 与 `integrity_check=ok` 保持，无 busy、migration、projection、重复学习或状态丢失。M5-P04 据此完成；M5-P05 继续关闭。
...
1. 冻结并保留 M5-P04 guest staging、backup、userdb、导入导出文件和 transient service 终态；未经单独授权不清理、不 reset、不覆盖。
2. M5-P05 继续关闭；后续若单独授权启动，先明确 Linux CJK 字体是随包交付还是由发行依赖保证，P04 的系统字体 fallback 实机通过不能替代安装产品依赖治理。
3. M5-P05 还需先固定用户级/系统级安装、升级、repair、remove、数据保留和 Fcitx 配置事务边界，再进入产品安装实现。
4. 旧临时资产清理、远端推送、tag/Release、真实同步和其他平台均不进入当前批次；对应动作继续单独授权。
