...
3. R01B 自动化代码批与实机前置门禁已经完成；提交 `f54763e` 的 `build 33` 已在 clean HEAD 使用版本化 `librime 1.17.0` 和哈希一致的隔离输入重跑 contract、ad-hoc native、精准测试与仓库门禁，五项产物哈希与提交前候选一致，源码、构建输入和 ad-hoc 产物已冻结。下一步进入第一阶段逐项授权，不在安装现场继续修改或重建。
