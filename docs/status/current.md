...
2. build 38 已完成标准 Application 菜单、`⌘Q`、版本传播、双 bundle/Installer/DMG 装配、自动门禁、`dev` 推送、远端 draft 三项资产替换及 Chrome 独立下载；下载副本的大小、SHA-256、逐字节比较、真实 quarantine、标准菜单、`⌘Q`、关闭最后窗口正常退出与 `prepared` 重启续跑均已人工复验。
3. 下一步使用同一 build 38 下载副本完成首次安装，再复验固定路径启动、输入 smoke、repair 与默认程序 remove；不得复用 build 37 的 bundle identity、载体摘要或实机成功断言。
