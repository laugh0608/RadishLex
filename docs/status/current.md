...
- 首台rollback clone `9FE6265D…AE28`从S3建立，`Network=[]`，文件回读`a18e1ab9…217f`确认仅loopback、双main route为空、package/receipt与进程静止；canonical pair/source/target身份不变。完整preflight脚本虽与宿主字节一致，guest-agent却只返回exit 0/空输出且result absent，故未通过、未重试。
- failed-closed host manifest `6b7ac641…a51d`与关机盘`fdfc0502…23a5`/`a73a3266…9155`/`2b323e27…bb4a`已冻结；S3未漂移、零句柄，十四台stopped。旧证据支持“push mode `0666`未归一化”这一高概率触发点，但精确predicate仍未知；已确认缺口是失败路径没有持久phase/terminal result。
...
- 首台rollback clone `9FE6265D…AE28`只作failed-closed取证，不启动、重试、诊断、恢复、清理或复用；不得把guest-agent exit 0、空输出或推定mode mismatch写成preflight通过或精确guest失败谓词。
...
1. 所有failure/mismatch/rolled-back、三台repair clone、首台rollback clone、S3及证据原样保留，不启动、重试、恢复、清理、复用或混用。
2. 下一次rollback preflight须另行授权且只从S3建立全新clone。传入物先在私有根归一化并复验`0600`/root/single-link/size/hash；检查须以`O_EXCL`落attempt marker，逐phase记录并在pass/fail原子写terminal result，宿主回拉验真。结果缺失继续失败关闭且不得原clone重试。
