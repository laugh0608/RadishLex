...
- L6 format v1、acceptance controller、release-pair 与 maintenance-only refresh v1 已完成，六类真实operation均有独立证据。首个`install_prepared`的startup缺陷已修复并在第三台clean retry闭合到关机；第二个case `install_artifacts_staged`的离线精确合同也已闭合。当前十八台VM全部stopped，连续完整L6与其余七个crash实机case尚未闭合。
...
1. `install_artifacts_staged`离线合同与回归已经闭合。下一步须取得clone-only授权，复验Git、十八台all-stopped、冻结manifest/handoff与DependencyFrozen后，只从未改写DependencyFrozen建立独立clean clone；不得启动或传input。
2. 后续启动/断网、input/preflight、controller checkpoint、是否resume与关机冻结继续逐笔授权；checkpoint必须精确命中`install_artifacts_staged/artifacts_staged`，任一receipt/staging/guard/package/dpkg/startup/XDG/process差异即保留现场且不自动进入下一步。
...
上述入口证明 prior-terminal anchor、repair/refresh、共享父目录策略及第二个case离线合同；真实repair、rollback、remove与reinstall均已闭合。仅首个crash实机case通过，第二个case尚未创建VM，八个case整体、连续完整L6、桌面启动/重启与公开发布仍未闭合。
