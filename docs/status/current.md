...
2. build 38 已完成标准 Application 菜单、`⌘Q`、版本传播、双 bundle/Installer/DMG 装配、自动门禁、`dev` 推送、远端 draft 三项资产替换、Chrome 独立下载与首次安装；下载副本、生命周期、`prepared` 续跑和安装后身份链均已复验。
3. 下一步复验 build 38 固定路径启动、输入 smoke、repair 与默认程序 remove；不得复用 build 37 的 bundle identity、载体摘要或实机成功断言。
