...
- L6 format/controller、release-pair与maintenance refresh已完成，六类operation均有独立证据。首个crash已闭合；`install_artifacts_staged`两次start均未进入guest，第二次由单次控制以timeout/全停合同返回10并冻结。十九台VM全部stopped，其余七个crash实机case和连续完整L6未闭合。
...
1. 新授权后先复验clean提交、`337007ff…7ebbb`、十九台全停及DependencyFrozen身份，只建立另一台独立clean clone并冻结UUID/config/EFI/qcow2/`Network=[]`；clone-only批次不得启动、传input、生成operation ID或运行controller。
2. 新clone启动与首条guest断网继续单独授权；只有target started、其余全停且双重文件回读证明仅`lo`/双main route为空，才可再分批进入input/preflight。checkpoint仍须精确命中`install_artifacts_staged/artifacts_staged`，任一receipt/staging/guard/package/dpkg/startup/XDG/process差异即保留现场。
3. 是否resume、关机冻结、第二批repair清理、连续完整L6、guest reboot、dynamic preload/错误sibling、P05C、发布、推送、真实同步及其他平台继续关闭；不得自动推进或用分散证据冒充完整session。
...
上述入口以合成执行器证明单次start、四类terminal和零自动stop/retry，不调用真实UTM。第二个case两次start均未进入guest；八case整体、连续完整L6与发布未闭合。
