...
- UI 已拆分 manager shell、跨页 action 编排、词库子组件、学习子组件、同步子组件、设置诊断子组件、页面级 widget tests、Dart model 分组和动态 FFI bridge 分层。
...
2. 继续 Phase 4 manager 本地质量治理；学习页和同步页子组件已拆分，下一步优先审视 `manager_home_actions.dart` 是否需要继续按职责拆分或补针对性回归测试。
