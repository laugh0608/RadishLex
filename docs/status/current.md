...
- production 代码与合成门禁已闭合；L6 format v1、compile-isolated acceptance checkpoint/evidence controller、六步主序列、八点确定性 crash/recovery 合成矩阵与 canonical 脱敏 evidence 已完成。2026-08-09 使用旧 source `55351f2`/target `e5b6da1` pair 的首次授权 install 在真实 Debian 13 默认 `dpkg.cfg` validation 处 pre-receipt 失败关闭：未调用 `/usr/bin/dpkg`，package/receipt/operation/guard/tmp 与产品路径均 absent。修复已进入 target `2fa1b8c`；新 pair、独立 guest、root-owned handoff 与 `S0-clean-2fa1b8c-5683d120` 已复验。旧 failure L6 继续保持运行且离线，新 L6 与 builder 均停止；真实用户同步继续关闭。
...
- `bb84d4a` 将真实 Debian 13 默认 `no-debsig` 与固定 `/var/log/dpkg.log` 的空白/等号写法规范化为受控配置；其他日志路径、额外参数、未知选项及 `root`/`admindir`/`instdir`/`force-*`/hook/path include-exclude 继续失败关闭。target `2fa1b8c` 的 production maintenance identity 已由真实 ARM64 ELF `919b55dc46958ca55520e0cb9a0fae5c8080f9b58e1bd9142b662af10b7c48eb` 冻结。
...
- 新 release-pair 固定 source `55351f2`/revision 1 与 target `2fa1b8c`/revision 2；record SHA-256 为 `a5a0ee0deeb48a1e82e87e0bb9eb848e118d21c83274dc2689fcc136dcb38664`。source/target package 分别为 `08205ad712ea7bde08b19a56e42c42ce0c15440f61fae2efd610d024188c0ad1`、`44e0f3da48502dad7d4ea22eb05e0e87f77dfddf98910c098e8f536128ecca42`，production/acceptance ELF 分别为 `919b55dc46958ca55520e0cb9a0fae5c8080f9b58e1bd9142b662af10b7c48eb`、`29c2007b860e506a4ee1dbfe2608e4e18bd5e37edb5c3bbe8fddd4c3b3e2e867`；guest、host 与 final handoff 逐哈希一致。
- 新执行 VM 为 `/Users/luobo/VirtualMachines/Debian13-ARM64-L6-2fa1b8c.utm`，UTM UUID `EBF12F50-33B1-4711-B693-B57D419EAE2A`，当前 stopped；root-owned `/var/tmp/radishlex-l6-inputs` 已绑定新 pair。未注册 S0 位于 `RadishLex-L6-Snapshots/S0-clean-2fa1b8c`，identity `S0-clean-2fa1b8c-5683d120`，local evidence SHA-256 `d741d07b90b554bd77b4b2a66447fe142c95b635064bbd9aa46b04e333d23f7c`；disposable 复验后 source/S0 三项 hash 未漂移，package/state/evidence 与五个 XDG 路径均 absent。
...
- 不停止、恢复、清理或复用当前 disposable L6 failure 现场；旧 pair/handoff/S0 只保留取证，不得在原处替换 maintenance executable 后重试。
...
1. 保持旧 L6 failure overlay 运行且离线，P04、CleanBase、DependencyFrozen、builder 与新 L6 继续关机；不清理旧空 state root、不恢复旧 S0。
2. 另行授权以 disposable 模式启动新 L6 `EBF12F50-33B1-4711-B693-B57D419EAE2A`，关闭其网络并只读复验新 S0/pair/dpkg/XDG/依赖；旧 L6 不停止，新 handoff 不替换。
3. preflight 通过后，以新的 operation ID、精确 argv 和单步授权重做 source revision 1 首次 install；terminal receipt、guard/tmp、dpkg status/audit、依赖/字体、完整 inventory、startup decision 与五个 XDG absent 未闭合前不启动产品。
...
上述仓库入口本身只证明合同与合成状态。新 canonical ARM64 pair、独立 guest/handoff/S0 已冻结修复后的 package/executable identity，但新 maintenance/acceptance CLI 尚未调用；旧 production CLI 的一次调用没有形成 receipt，也未调用 `/usr/bin/dpkg`。当前仍不证明 process-group/dpkg mutation、桌面安装、L6 完成或公开发布。
