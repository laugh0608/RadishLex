...
- `9300ed7`闭合原生Move恢复：prepare只读绑定、UI Move独立、complete仅在身份不变后单次typed update；8项与完整L6通过。
- prepare v1首门拒绝且list为0；v2以一次list闭合22台全停及15项`move-ready` manifest `dc6b0d01…738e`，Move/update为0。
...
- complete v1因跨阶段HEAD误绑在首门闭合`state-indeterminate`；2项manifest `1fd1dc94…ceb7`、list/update均为0。`9da8d0f`修复并通过8项及完整L6门禁。
...
- v4、prepare v1/v2与complete v1根冻结；部分壳位于既定外部package，默认路径和`Evidence-v5`均absent。下一步只允许新v2 complete授权；不得手工修补、update/delete/start/clone/retry/query。
...
1. prepare v2和唯一UTM UI Move已完成；默认路径absent，外部三文件身份不变。
2. complete v1首门失败且零UTM调用；`9da8d0f`已修复跨阶段HEAD绑定并通过完整L6门禁。
3. 下一步另行授权v2 complete的两次list与一次stopped typed update；壳体manifest前关闭clone及后续五段。
