...
- L6 format v1、acceptance controller、release-pair 与 maintenance-only refresh v1 已完成，六类真实operation均有独立证据。首个`install_prepared` checkpoint暴露的startup共享锁目录漂移已修复，新pair与离线guest-case合同已冻结；第三台clean retry clone已创建但未启动，连续完整L6与八个crash/retry尚未闭合。
...
- 新`d75818f` handoff只允许作为独立clone的冻结输入；第三台clone不得无授权启动、传input或生成operation ID，也不得覆盖、热替换或与旧pair跨套混搭。
...
1. 下一笔系统动作须另行授权：只启动第三台clone；启动前确认其余十七台stopped，启动后第一项guest动作是关闭非loopback接口并以文件回读证明仅`lo`且IPv4/IPv6 main route为空。input与只读/mutation preflight通过前不生成operation ID或运行acceptance；`install_prepared`仍须下一笔独立授权。
