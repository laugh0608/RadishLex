...
- L6 format v1、acceptance controller、release-pair 与 maintenance-only refresh v1 已完成。第五套为 `rolled_back`，第六套 upgrade 为 target `completed`；旧 repair clone 的唯一旧 ELF 调用在 dpkg 前 `aborted_preserved`，源码已修复。`b891ed1` refresh handoff与新 clone断网只读preflight均已冻结；真实 repair尚未执行，十二台 VM 全停。
...
- maintenance-refresh v1 已锚定旧 record、target artifact/ELF 与 `b0197f5`；26 项合成测试拒绝 package rebuild、旧 ELF、acceptance、身份漂移和覆盖式/非原子发布。真实 ARM64 record/新 ELF 为 `4b41d1c0…fb2cd`/`9a657510…54585`，旧 target package/evidence 保持 `b211d940…d09c`/`2a1132c6…0e1b`。
- 十二台注册VM全部stopped；新 clone `BE3579E0…37F8` 的 local/guest evidence 为`7db3a16d…7502`/`4a92116f…1c62`，关机盘为`b06691d1…0ea9`/`a73a3266…9155`/`6267d82e…089d`。完整资产与流水只在L6 runbook和周志维护。
...
3. 新 clone已从S3建立、切入`b891ed1` refresh handoff并完成只读preflight；S3、旧pair/package与用户XDG未改写，十二台均stopped。
4. 下一步仍须另行授权新 clone 的单次真实 repair；其余矩阵继续关闭。
...
上述入口证明 prior-terminal anchor、target-only builder、canonical revision、single-target repair effective-source与production-only refresh合同。第三至六套依次覆盖source install、chain mismatch、target failure/recovery和target terminal；旧repair失败暴露并修复staged verifier缺口。refresh handoff与新clone只读preflight已形成；真实repair、其后主序列、八个crash case、桌面启动、重启、完整L6与公开发布仍未闭合。
