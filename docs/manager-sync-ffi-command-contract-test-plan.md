...
| L2 | Rust host contract test 计划 | 已补评审材料、source-level checklist、test design fixture、C ABI contract review matrix，并落地内部非导出 Rust draft module | 在新增 symbol 前先明确测试文件、输入样本、错误码、释放责任、既有 FFI 源码模式映射和 forbidden material 断言。 |
...
- Rust 内部非导出 draft module：`crates/ime-ffi/src/manager_sync_command.rs` 已覆盖 request parsing、action-specific section allowlist、bool / UTF-8 校验、当前阶段 `InvalidState` 摘要、result handle / release、error handle lifecycle、panic boundary、sync domain guard 和 forbidden material 拒绝；仍不导出 C ABI symbol。
