...
- `apple-secure-enclave-p256-v1` 已完成独立 repository、FFI、manager native、qualification bundle 与产品 lifecycle；实测支持 runtime、不可导出和 hardware-backed，产品资格仍等待 denied/locked/unsupported 失败矩阵。
...
| `apple-secure-enclave-p256-v1` | 产品 runtime、不可导出与 hardware-backed 已取证，产品资格关闭 | qualification manager 产品进程已完成创建、重载、签名、Rust/Go 验签、private export-block、删除、missing 和 cleanup | 单独授权 denied/locked/unsupported；完成失败矩阵后评审 product qualification，user presence/backup migration 继续独立关闭 |
...
5. 已补 Secure Enclave 独立 backend ADR/runbook，并沿 crypto、FFI、manager native 完成 repository 接线、qualification bundle 与 lifecycle；unsupported 设备失败关闭，不回退普通 DPK 或 test memory。capability 更新后的 HEAD 需重新冻结，再按独立授权补 denied/locked/unsupported smoke。
