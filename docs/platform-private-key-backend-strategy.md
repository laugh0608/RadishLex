...
5. 已补 Secure Enclave 独立 backend ADR/runbook，并沿 crypto、FFI、manager native 完成 repository 接线、qualification lifecycle 与 capability 更新后 bundle 重冻结；unsupported 设备失败关闭，不回退普通 DPK 或 test memory。下一证据是独立授权的 denied/locked/unsupported smoke。
