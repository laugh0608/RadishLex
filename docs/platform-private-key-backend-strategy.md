...
| `apple-secure-enclave-p256-v1` | 产品 runtime、不可导出、hardware-backed 与 denied 已取证，产品资格关闭 | qualification lifecycle 与 ad-hoc denied 通过；经典登录 Keychain 锁定不能阻断 DPK/SE 签名，cleanup 已确认 missing | 使用延迟产品 probe + 手动锁屏验证真实设备锁定态；另需无 Secure Enclave 环境执行 unsupported |
...
5. 已补 Secure Enclave 独立 backend ADR/runbook，并沿 crypto、FFI、manager native 完成 repository 接线、qualification lifecycle、capability 重冻结与 ad-hoc denied；qualification bundle 已恢复，不回退普通 DPK 或 test memory。下一证据是分阶段执行 locked，并在无 Secure Enclave 环境执行 unsupported。
