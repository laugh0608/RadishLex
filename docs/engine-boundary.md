...
当前目标是进入 Phase 1：Rust Core 原型。该阶段应先让 `ime-core` 拥有可测试的输入会话、候选模型、提交模型和 engine interface，再推进真实 engine adapter 或平台壳。
...
- 后续 `ime-engine-rime` 只能通过 `ime-core` 暴露的 engine boundary 与 core 通信。
...
该验证只证明 Rust core 边界可复验，不证明真实中文输入质量、`librime` adapter、用户词库、ranker 或平台输入法可用。
