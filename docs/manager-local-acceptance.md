...
| 产品 bundle 可加载真实 Rust 能力 | Xcode 构建阶段嵌入 native library，校验 ABI v8、owner-thread、panic boundary、架构、必需符号和依赖；Release bundle smoke 使用其中的 dylib，并验证隔离资格 run 的失败清理与释放。 | `embed-manager-native-library.sh`、`check-manager-product.sh`、`ffi_dynamic_native_binding.dart` |
