...
8. 已补 `ime-sync` remote object client DTO / transport trait，上传入口只接收 `AssembledSyncObject` 和 `SignedSyncObjectManifest`，用于验证 Go server JSON / base64 metadata、binary payload 下载和错误映射；真实 HTTP transport 后置。
