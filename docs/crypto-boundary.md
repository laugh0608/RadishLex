...
- 设备签名与私钥存储边界已由 ADR 0003/0006/0007 固定并在 Rust 模型中落地：`ed25519-v1` 与 `ecdsa-p256-sha256-v1` 按显式 profile 共存，签名 key 只用于签名。普通 DPK P-256 运行时已验证但因 `exportable=true` 被拒绝生产资格；独立 `apple-secure-enclave-p256-v1` 已完成 qualification lifecycle、不可导出、hardware-backed、ad-hoc denied、真实设备锁屏 locked 与 cleanup，并按受支持 macOS 主路径取得产品资格。unsupported 延期补测，所有 backend 仍禁止 fallback。
...
8. 已补 `ime-sync` remote object client DTO / transport trait 和 `http://` / rustls `https://` transport，上传入口只接收 `AssembledSyncObject` 和 `SignedSyncObjectManifest`，用于验证 Go server JSON / base64 metadata、binary payload 下载、真实 HTTP/TLS request / response 传递和错误映射。HTTPS 固定验证证书链与主机名，不提供 insecure bypass；本地 CA 只允许作为进程内 DER trust anchor。
...
24. 已补 sync server 备份恢复、外部 TLS 反代和升级回滚实现级验证证据；macOS Secure Enclave P-256 signing/key-agreement 已按支持设备主路径通过产品资格，进入用户可用同步前仍需产品入口退出评审，并按首版后计划补目标部署运行证据。Apple 原生非导出 Ed25519 支持矩阵继续作为独立历史结论。
