...
- 已进入 `ime-crypto` 本地 crate 的设计与测试准备，当前覆盖 XChaCha20Poly1305、HKDF-SHA256、SHA-256 ciphertext hash、envelope、key role、AAD、nonce 和篡改失败测试；`ime-sync` 已可从 crypto envelope 派生上传草案元数据。
...
5. 再接入 userdb 的 P2 plaintext payload 导出迭代器，仍不连接后端。
