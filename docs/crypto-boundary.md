...
20. 已按 `docs/sync-server-api-storage.md` 起步 Go server metadata / storage / API 验证模型，覆盖配置默认值、API request / error DTO、SQLite migration 文本、storage interface、storage conformance tests、内存 storage、local object storage staged transaction、版本冲突、撤销设备阻断和隐私字段检查。
21. 下一步继续推进 SQLite-backed metadata repository、metadata transaction 与 local object storage transaction 接线、签名、版本冲突和错误语义验证。
