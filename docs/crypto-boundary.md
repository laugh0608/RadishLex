...
- 设备签名与私钥存储边界已由 ADR 0003/0006/0007 固定并在 Rust 模型中落地：`ed25519-v1` 与 `ecdsa-p256-sha256-v1` 按显式 profile 共存，签名 key 只用于签名。普通 DPK P-256 运行时已验证但因 `exportable=true` 被拒绝生产资格；独立 `apple-secure-enclave-p256-v1` 已完成 compiled-only repository/FFI/manager native 接线，runtime、hardware-backed 与 product qualification 等待产品环境证据。所有 backend 均禁止 fallback，既有 Apple/Android Ed25519 阻塞也未解除。
