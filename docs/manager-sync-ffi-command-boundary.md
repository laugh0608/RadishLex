...
当前 host test file approval review 将 `crates/ime-ffi/tests/manager_sync_command_boundary.rs` 真实文件创建前的计划测试项、symbol lookup、capability missing、result / error handle 生命周期、panic boundary、forbidden material 断言和 dynamic library smoke 缺席证据固定为评审材料，仍不创建该文件。当前 real sync execution gate review 将 owner / worker、sync domain serialization、operation id、readiness snapshot、deployment evidence、platform private key backend、恢复 / 授权实现测试和发布级部署证据固定为真实执行前 gate；当前仍不能连接 Go server、触碰平台 key backend、生成恢复码、创建 join request、撤销设备或执行真实同步。当前 real sync execution evidence bundle review 再把平台私钥 backend、恢复 / 授权交互测试和发布级部署摘要三类证据收敛成非敏感 evidence item，逐项记录 current state、blocking condition、required source 和 safe summary；`local_smoke` 只能解释开发期联调，不会替代 `deployment_evidence_summary.v1` 或解除真实同步 gate。
...
当前 Dart fake native binding 已用该 catalog 做非执行 replay：逐个 host input sample 生成测试侧 request / result 摘要，反查 ABI status、input case、required evidence、copy -> free 顺序和 command error allowlist；同时新增 host gate migration readiness replay、host test admission gap replay、native export approval replay、Dart binding migration replay、ManagerBridge migration replay、host test file approval replay、real sync execution gate replay 和 real sync execution evidence bundle replay，覆盖当前全部阻塞、native symbol 未批准、Dart binding 未批准、`ManagerBridge` command 未批准、host test 文件未批准、dynamic library smoke 缺席、平台私钥 backend 未达生产、恢复 / 授权实现测试未通过、发布级证据不足、release evidence bundle 未批准和真实同步执行未批准等场景。该 replay 只验证 Dart mapper 的脱敏与降级边界，不代表真实 native command 已存在。
...
- host contract catalog 中的合成 ABI status / input case / required evidence、host gate migration readiness 阻塞项、host test admission gap item、native export approval review、Dart binding migration review、ManagerBridge migration review、host test file approval review、real sync execution gate review 和 real sync execution evidence bundle review，能被测试侧 fake native binding 重放并保持非敏感摘要。
