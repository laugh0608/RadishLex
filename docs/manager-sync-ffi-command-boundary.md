...
- 不新增 `ime-ffi` C ABI symbol、Rust FFI handler 或 Dart native binding；当前仅允许 `ime-ffi` 内部非导出 draft module、C ABI wrapper 形状评审和 host contract test gate 草案。
...
内部 command context owner scope 草案当前只允许 Rust-owned context 持有 active domain set 和创建线程标识；它不持有 Flutter widget state、settings draft payload、Dart pointer 或平台 UI object。跨线程直接使用该 context 会返回结构化 `InvalidState`，后续如果引入专用 command worker，必须重新评审 owner 迁移、消息边界和取消语义，不能把 Flutter 临时状态或 secret payload 排队到 Rust context。
...
- host contract catalog 中的合成 ABI status / input case / required evidence 能被测试侧 fake native binding 重放并保持非敏感摘要。
...
- C ABI wrapper shape review 继续显示候选 symbol 未导出，host contract test gate 已通过 ADR / 文档 / smoke 证据明确从关闭态迁出。
