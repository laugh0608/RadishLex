...
18. 已补 Docker Compose 本地 / 部署态入口、sync server Dockerfile、Docker build context ignore、本地 Caddy HTTPS 入口、部署态 HTTP 上游、Nginx 生产反代示例和 `docs/runbooks/sync-server-compose.md`；本地默认 `https://localhost:7443`，部署态默认同机 HTTP 上游 `http://127.0.0.1:7319`，并明确生产认证 / 备份 / 平台私钥 backend 未补齐前不得开放给真实用户。
