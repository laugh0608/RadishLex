...
18. 已补 Docker Compose env 切换入口、sync server Dockerfile、Docker build context ignore、Caddy local / production 入口和 `docs/runbooks/sync-server-compose.md`；本地默认 `https://localhost:7443`，部署态默认同机 HTTP 上游 `http://127.0.0.1:7319`，使用命名 volume 保存 SQLite metadata 与 encrypted blobs，并明确生产认证 / 备份 / 平台私钥 backend 未补齐前不得开放给真实用户。
