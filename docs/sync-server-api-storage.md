...
18. 已补 Docker Compose 本机运行入口、sync server Dockerfile、Docker build context ignore 和 `docs/runbooks/sync-server-compose.md`，默认只绑定 `127.0.0.1:7319`，使用命名 volume 保存 SQLite metadata 与 encrypted blobs，并明确生产 TLS / 认证 / 备份 / 平台私钥 backend 未补齐前不得开放给真实用户。
