...
- 平台私钥存储 backend 能力模型已落地；`apple-keychain-v1` 仍阻塞，`apple-keychain-p256-v1` 已通过命令行基础生命周期 Keychain smoke，但尚未完成产品进程访问、失败矩阵与 capability status 评审。可用平台 backend 验证未完成前，不提供用户可用同步 UI。
