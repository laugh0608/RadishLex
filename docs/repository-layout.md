...
`packaging/linux/l6-matrix.json` 与 `scripts/linux-product/l6_contract.py` 固定独立 Debian 13 ARM64 guest、不同 commit 的相邻 Debian revision、六步主序列、八个 crash checkpoint、字体/startup/XDG/procfs probe 和逐 mutation 授权；controller 合同另固定 compile identity、进程组终止与 evidence 边界。release-pair builder 以 `l6_source_anchor.py` 精确冻结 committed prior-terminal source package/evidence，只从 clean target 构建载体与双 executable，再运行 target production Rust verifier 逐侧解析 `.deb`。第四套在 operation ID/CLI 前确认 source chain 不连续；第五套只执行一次 upgrade，target validation 失败后自动恢复 source并形成 terminal `rolled_back`。离线回归已统一 canonical release 门禁；修复 commit `80e49ce` 的第六套 handoff `cda70afa…659b` 已断网构建、双 verifier 复验并原子发布，独立 S2 clone 也已在全程 loopback-only/零路由边界完成只读 preflight。target terminal、其余主序列和 crash matrix 尚未闭合，下一步只在独立授权中执行一次第六套 source→target upgrade，不复用第五套现场。
