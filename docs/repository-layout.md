...
`packaging/linux/l6-matrix.json` 与 `scripts/linux-product/l6_contract.py` 固定独立 Debian 13 ARM64 guest、不同 commit 的相邻 Debian revision、六步主序列、八个 crash checkpoint、字体/startup/XDG/procfs probe 和逐 mutation 授权；controller 合同另固定 compile identity、进程组终止与 evidence 边界。现有 release-pair builder 在两个 clean root 构建真实 pair，除 Python artifact/ELF/envelope 校验外，还运行 target production Rust verifier 逐侧解析 `.deb`。前三套真实 pair 已依次暴露两个环境缺口与一个 revision profile 缺口；第四套真实 pair/clone/input/断网 preflight 完成后，在 operation ID/CLI 前确认重建 source 与 S2 terminal artifact chain 不连续并零 mutation 停止。下一步为 builder/verifier 增加 prior-terminal source anchor 并冻结新 handoff；新的 upgrade、其余主序列和 crash matrix 尚未闭合。
