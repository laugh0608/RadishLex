...
当前已起步 `server/sync-server/`，但只实现 metadata / storage / API 验证模型、SQLite-backed metadata repository、local object storage staged transaction、对象 payload hash / length 复验、Ed25519 签名验签门禁、device wrapping encrypted key bytes 承载、recovery wrapped material 读取、recovery latest handler、domain / device / join request metadata handler、authorization handler、request id、panic recovery、非持久审计 hook 和 SQLite audit_events 写入。后续应进入对象上传下载前错误语义；真实远端上传下载、Rust 远端客户端、Flutter manager 和平台壳继续后置。
