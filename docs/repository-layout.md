...
| 平台 | macOS InputMethodKit 薄壳、contract/native bundle、生产 LearningContext 与 privacy/清理 contract；build 38 双 bundle、locked RimeData、数据/安装 gate、Installer、社区 ad-hoc identity、DMG evidence、首次安装/输入/修复/默认移除实机证据；Linux Fcitx5 C++/CMake addon、ABI/XDG/staged/system runtime-layout/Manager runtime/privacy/classifier contract、Debian 13 ARM64 staged 装配/headless loader、Wayland/X11 输入/隐私及 Manager 同库个人化实机证据；Linux metadata/rootfs、真实 ARM64 product payload、确定性 `.deb`、receipt/guard、fake dpkg 事务与 Manager/Fcitx 共用只读 startup gate；Android Keystore 能力验证桥 | macOS 真实跨发布升级；Linux mutable dpkg adapter、maintainer scripts、依赖/版本关系复验、隔离 matrix 与独立安装实机；完整 Android IME、Windows TSF 与 iOS Keyboard Extension |
...
`platforms/linux-product/` 是 M5-P05B 的系统 package 事务与只读 startup decision 层。Rust crate 固定 canonical receipt、operation chain、source/target artifact staging、root/state identity、Unix socket guard、五类 operation 与恢复状态机；`DpkgTransactionPort` 隔离 package/environment inspection、quiescence、mutation 与 source restore。startup observer 直接读取 `/var/lib/dpkg/status`，只读复验 guard/tmp/receipt、terminal staging proof 与 package 状态；Manager scope 覆盖完整 bundle tree，Fcitx scope 覆盖 addon/FFI/RimeData/两份 metadata，并验证两份 FFI equivalence、owner/mode/link/hash、ABI 与 data contract。Manager 与 Fcitx 通过同一 additive request/result v1 FFI，在 Flutter application/engine 或 Engine/input FFI/XDG/Rime 初始化前消费 decision；C++ binding 用 `dladdr`/canonical path 拒绝非精确 sibling startup/error/input symbol origin，`development-staged` 与 `debian-system-product` 是编译身份。observer 不调用 `dpkg`、不创建或清理 state、也不读取 XDG/home；普通门禁只在 canonical 临时目录使用合成 artifact 和 fake port，不启动 Manager/Fcitx，也不写 `/usr`、`/var` 或现有 guest。startup scope 不冒充全 package runtime inventory；mutable dpkg adapter、maintainer scripts、外部 dependency/font/version relationship validation、privileged maintenance command 与 Debian ARM64 L6 matrix 仍是后续职责。
...
1. `platforms/linux-fcitx5/` 与 `platforms/linux-product/`：前者承载 M5-P02-P04 输入/Manager 主线、system runtime profile 与共用 C++ startup binding，后者已承载 P05B receipt/guard、五类 operation、fake dpkg 事务核心与只读 startup decision；确定性 `.deb` 仍归属 `packaging/linux/` 与 `scripts/linux-product/`，当前继续 mutable dpkg adapter、依赖/版本关系与 maintainer-script 投影。
