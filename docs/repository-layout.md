...
| 本地学习 | schema v6 userdb、事务化用户意图、本地导入批次关联、确定性 ranker、产品热路径、并发 migration/WAL、同步 cursor/journal/outbox、原子 apply 与可信 public lifecycle cache | M3 wrapped epoch material 只由平台受保护边界承载；不得保存明文 master key |
| 同步 | P2 crypto/sync、Ed25519/P-256 profile、Go server、Rust HTTP transport、关闭态 orchestration、通用 processor、生产 provider 三端口、设备 lifecycle 验证与 public cache、双 userdb Go HTTP 收敛、Apple DPK/Secure Enclave backend | wrapped material/backend 真实 adapter、Secure Enclave 最终资格与发布级部署 |
