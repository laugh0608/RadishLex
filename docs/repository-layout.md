...
`packaging/linux/l6-matrix.json` 与 `scripts/linux-product/l6_contract.py` 固定独立 Debian 13 ARM64 guest、不同 commit 的相邻 Debian revision、六步主序列、八个 crash checkpoint、字体/startup/XDG/procfs probe 和逐 mutation 授权；controller 合同另固定 compile identity、进程组终止与 evidence 边界。release-pair builder 以 `l6_source_anchor.py` 精确冻结 committed prior-terminal source package/evidence，只从 clean target 构建载体与双 executable，再运行 target production Rust verifier 逐侧解析 `.deb`。第四套确认 source chain 不连续；第五套 upgrade 自动恢复 source并形成 `rolled_back`。第六套已完成 target `completed` 与 S3；首次 repair 在 staged preflight `aborted_preserved`，暴露并修复 production system port 的 target-only effective-source 缺口。`l6-maintenance-refresh.json` 与 `l6_maintenance_refresh*.py` 另行锚定旧 pair/target artifact、repair fix祖先及production-only ELF，builder不重建package、不产出acceptance并由Python执行exclusive staging、原子rename和发布后重验。`b891ed1` ARM64 handoff、新clone只读preflight及唯一maintenance调用均已冻结；该调用虽形成`repair/same_release/completed` receipt，却因健康product validation提前返回而未进入dpkg，不能作为repair重装证据。下一步先区分首次健康repair与已有target proof的crash retry，真实repair、主序列和crash matrix仍未闭合。
