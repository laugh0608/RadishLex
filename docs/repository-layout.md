...
`packaging/linux/l6-matrix.json` 与 `scripts/linux-product/l6_contract.py` 固定独立 Debian 13 ARM64 guest、不同 commit 的相邻 Debian revision、六步主序列、八个 crash checkpoint、字体/startup/XDG/procfs probe 和逐 mutation 授权；controller 合同另固定 compile identity、进程组终止与 evidence 边界。release-pair builder 以 `l6_source_anchor.py` 精确冻结 committed prior-terminal source package/evidence，只从 clean target 构建载体与双 executable，再运行 target production Rust verifier 逐侧解析 `.deb`。前三套真实 pair 暴露两个环境缺口与 revision profile 缺口；第四套在 operation ID/CLI 前确认重建 source 与 S2 chain 不连续并零 mutation 停止；第五套 target `1ebbdab` handoff 与独立 S2 clone 已执行一次 upgrade，target validation 失败后自动恢复 source，terminal 为 `rolled_back`。离线回归已定位 startup installed-product 的 revision `1` 重复假设并统一 canonical release 门禁；target terminal upgrade、其余主序列和 crash matrix 尚未闭合，下一步重建 pair/clone 且不复用第五套现场。
