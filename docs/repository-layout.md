...
- `platforms/android-ime/keystore-bridge/`：Android Keystore bridge 仓库内 Kotlin / Gradle harness，固定 `android-keystore-v1` 的 `AndroidKeyStore` / `Ed25519` 创建、加载、公钥读取、签名、删除、`@JvmStatic` facade、gated instrumented smoke 和 smoke 记录模板；Rust raw JNI glue 位于 `crates/ime-crypto`，该目录当前不包含真实设备 smoke 结果或完整 Android IME。
