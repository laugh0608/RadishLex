...
`packaging/linux/l6-matrix.json` 与 `scripts/linux-product/l6_contract.py` 固定独立 Debian 13 ARM64 guest、不同 commit 的相邻 Debian revision、六步主序列、八个 crash checkpoint、字体/startup/XDG/procfs probe 和逐 mutation 授权；`scripts/linux-product/l6_controller_contract.py` 另固定 compile identity、进程组终止、无 runtime override 与 evidence 字段边界。`packaging/linux/l6-release-pair.json` 冻结 source commit/revision、target revision、共享 data contract 和两个 executable build profile；`scripts/build-linux-l6-release-pair.sh` 在两个独立 clean root 构建真实 pair，`l6_release_pair.py` 强校验两份 `.deb`、AArch64 ELF/loader/marker 并形成 canonical 脱敏 envelope，`check-linux-l6-release-pair.sh` 只运行合成合同。执行说明位于 [Linux L6 Debian package matrix runbook](runbooks/linux-l6-package-matrix.md)；source `55351f2` 与 target `e5b6da1`/`2fa1b8c` 的两个真实 ARM64 pair、独立 guest、handoff 与 S0 已分别暴露 pre-receipt 环境缺口；包含 `f0415ad` 的 target `512e8ab` 第三套输入已真实完成 source install 并冻结 S1，但 S2、其余五步和 crash matrix 尚未执行，不能把单次 terminal 结果写成 L6 通过。
