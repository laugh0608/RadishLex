...
| 范围 | 已有工程形态 | 尚未形成的产品能力 |
| --- | --- | --- |
| Rust input | core、进程级 Rime runtime、产品个人化 runtime、CLI、ABI v8、Manager 产品状态与隔离资格 run、管理查询和共库证据；M4-P02 数据协调与 M4-P03 外层安装 receipt/guard、产品身份、startup decision 已形成独立核心 | M4-P03 程序切换/恢复、macOS 安装 adapter 和发布复验 |
