...
startup observer 只读 `/var/lib/dpkg/status`、guard/tmp/receipt、terminal actual package/dependency relationship 与 component identity；Manager/Fcitx 在业务初始化前通过 additive request/result v1 取得 decision，C++ binding 以 `dladdr`/canonical path 拒绝错误 sibling symbol。production system 已在彼此隔离的 L6 guest 形成source/target terminal、自动source restore、repair/rollback/remove/reinstall、`/proc`静止、startup与XDG零漂移证据；这些分散证据不冒充同一连续session。fake command/crash matrix仍只使用actual合成artifact、fake executor/observer与临时目录，不写`/usr`、`/var`、XDG或既有guest。
...
`packaging/linux/l6-matrix.json` 与 `scripts/linux-product/l6_contract.py` 固定独立 Debian 13 ARM64 guest、相邻 revision、六步主序列和八个 crash checkpoint；`l6_guest_case_contract.py`固定guest input/readback与状态预期，`l6_utm_start_once.py`把host start限制为绑定clean head/失败manifest/全停VM的单次调用，逐次持久化有界诊断、status和terminal list。对称的`l6_utm_clone_once.py`绑定clean head/前序manifest/canonical全停清单/registered source与目标package absent，只调用一次clone并联合命令、terminal注册清单和精确package判定结果；两者都不自动补救或进入guest。`l6_utm_launch_diagnostics.py`只允许list/status、紧凑host process和最长15分钟的UTM/utmctl/QEMU unified log读取，`l6_utm_launch_diagnostic_bindings.py`独立负责clean head、双控制identity以及start/failure/postverify/v1-v6前序诊断manifest的hash与终态语义；普通命令保持64KiB捕获，仅日志使用8MiB上限并受事件/单行/消息/脱敏总量门约束。输出为有界脱敏create-new evidence，永不调用start/stop/clone/guest或宣称根因。v7 manifest `206aa335…7b56c`固定UTM/AppKit断言后，`l6_utm_launch_transport_v2.py`与通用bindings把新transport限定为committed AppleScript的单次`activate`+UUID start，`l6_utm_launch_prepared_bindings.py`独立绑定prepared七项证据、v7+唯一v4 target关系、authoritative package路径与descriptor live config/EFI/qcow2身份；两次进程零门后的target身份必须一致，才可进入osascript。canonical input由`l6_v4_canonical_input_bundle.py`从冻结handoff/template/prepared证据确定性生成host USTAR，再由`l6_utm_canonical_input_transfer.py`与guest installer绑定精确source三元组、双重回读、私有staging和no-replace switch；builder本身不调用UTM或进入guest。第六套已完成 target `completed` 与 S3；独立clone闭合真实repair、rollback、remove与reinstall。第三台clean retry闭合首个crash；第二个case的v4已进入guest并闭合断网门，但canonical input尚未传输。
