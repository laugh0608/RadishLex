...
`packaging/linux/l6-matrix.json` 与 `scripts/linux-product/l6_contract.py` 固定独立 Debian 13 ARM64 guest、不同 commit 的相邻 Debian revision、六步主序列、八个 crash checkpoint、字体/startup/XDG/procfs probe 和逐 mutation 授权；controller 合同另固定 compile identity、进程组终止与 evidence 边界。release-pair builder 在两个 clean root 构建真实 pair，除 Python artifact/ELF/envelope 校验外，还运行 target production Rust verifier 逐侧解析 `.deb`。前三套真实 pair 已依次暴露两个环境缺口与一个 revision profile 缺口；修复后的第四套真实 pair、独立 S2 clone、root-owned input 与断网 preflight 已冻结。新的 upgrade、其余主序列和 crash matrix 尚未闭合。
