...
`packaging/linux/l6-matrix.json` 与 `scripts/linux-product/l6_contract.py` 固定独立 Debian 13 ARM64 guest、不同 commit 的相邻 Debian revision、六步主序列、八个 crash checkpoint、字体/startup/XDG/procfs probe 和逐 mutation 授权；`l6_guest_case_contract.py`另固定canonical input、环境文件名、guest文件回读真相源与fresh-absent/remove-terminal互斥预期，controller 合同固定 compile identity、进程组终止与 evidence 边界。release-pair builder 以 `l6_source_anchor.py` 精确冻结 committed prior-terminal source package/evidence，只从 clean target 构建载体与双 executable，再运行 target production Rust verifier 逐侧解析 `.deb`。第四套确认 source chain 不连续；第五套 upgrade 自动恢复 source并形成 `rolled_back`。第六套已完成 target `completed` 与 S3；独立clone又完成真实repair、rollback、remove与reinstall，六类operation均有独立证据且XDG零漂移。首个`install_prepared`因startup共享锁父目录合同漂移在resume/dpkg前失败关闭；`a6622d4`统一策略后，修复target `d75818f`第三台clean retry已闭合input/preflight、prepared checkpoint、exact resume terminal与关机冻结，其余七个crash case仍未闭合。
