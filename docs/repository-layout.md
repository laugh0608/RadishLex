...
1. `platforms/linux-fcitx5/`、`platforms/linux-product/` 与 `platforms/linux-l6-acceptance/`：前者承载 M5-P02-P04 输入/Manager 主线、system runtime profile 与共用 C++ startup binding；中者承载 actual `.deb` relationship、恢复型 receipt/advisory guard、fixed-path observer/executor、concrete mutable port、`/proc` 静止、受控 CLI 与只读 startup decision；后者只承载 compile-isolated L6 checkpoint/process-group/evidence controller。确定性载体、release-pair、maintenance-refresh identity 与 L6 matrix 归属 `packaging/linux/`，验证/构建辅助归属 `scripts/linux-product/`、`scripts/build-linux-l6-release-pair.sh` 与 `scripts/build-linux-l6-maintenance-refresh.sh`；前三套完成 source install/S1/S2并转为失败取证，第四套为 chain mismatch，第五套为 target-validation failure/recovery。第六套target terminal/S3、两台repair failure/noop现场、`823afca` refresh handoff、第三台真实repair、真实rollback、真实remove与真实reinstall均已冻结；源码和refresh ancestry合同已闭合首次repair/retry分流。首个`install_prepared`case已在resume/dpkg前失败关闭，startup共享目录策略修复后的`d75818f` pair已经冻结；第三台clean clone现已闭合注册、断网、canonical input与fresh-absent preflight，当前下一步仅是在新授权下单次重试该checkpoint，而非热替换旧pair、resume旧clone或重建source package。
