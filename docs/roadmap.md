...
- 同步状态查看。
- 隐私模式开关。
- 本地学习状态、rank explain 摘要和 sync preflight 摘要。
- 同步 UI 按 `docs/manager-ui-boundary.md` 显示不可用原因，不绕过平台私钥 backend 和目标部署停止线。
- 当前 `apps/radishlex-manager` 已起步 macOS Flutter 工程，默认用合成 fixture 验证本地管理台结构；显式配置本地 SQLite userdb 与 `ime-ffi` 动态库时，可通过真实 Dart FFI bridge 读取 / 删除 / 导入 / 导出本地 userdb，并展示 learning status 与 sync preflight 摘要。
