...
- 进入 Phase 3 代码前，必须完成 `ime-crypto` 本地 envelope / key / hash 测试、同步密钥与设备生命周期 Rust 模型、删除合并测试和生产级 envelope 组装边界。
...
- 下一步先补客户端冲突合并、生产级 P2 payload envelope 组装边界、恢复码 KDF ADR 和签名 / 设备密钥存储设计，再进入远端同步实现。
