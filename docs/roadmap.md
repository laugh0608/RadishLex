...
- Phase 4 当前已补 `docs/status/current.md` 作为短入口，并新增 `docs/manager-local-acceptance.md` 映射本地验收范围、退出标准、验证入口、隐私检查和停止线；2026-07-05 复查确认本地验收当前无影响退出标准的证据缺口。后续不在可用平台私钥 backend 与目标部署运行证据齐备前打开真实远端同步、恢复码或设备授权 UI。
