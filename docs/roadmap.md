...
- 进入 Phase 3 代码前，必须完成 `ime-crypto` 本地 envelope / key / hash 测试、同步密钥与设备生命周期 Rust 模型、删除合并测试、生产级 envelope 组装边界、恢复码 KDF ADR、恢复码 KDF Rust 模型、签名 / 设备密钥存储 ADR 和签名 / 设备密钥存储 Rust 模型；随后补齐合并模型与真实 P2 payload / userdb 写回流程的接线。
...
- 实现设备注册。
- 实现备份恢复。
- 同步密钥、设备授权、撤销、key epoch 和签名模型的 Rust 测试已经完成。
- 下一步先补客户端合并模型与真实 P2 payload / userdb 写回流程的接线，再进入远端同步实现。
