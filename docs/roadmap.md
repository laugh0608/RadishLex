...
- Go server API / storage、生产恢复流程和平台私钥存储 backend 边界已由专题文档固定；平台私钥存储 backend capability / unavailable backend 的 Rust 模型和测试已经落地。Go server 已起步 metadata / storage / API 验证模型，下一步若进入代码，应补 SQLite-backed repository、local object storage transaction、签名、版本冲突和错误语义验证。真实远端上传下载后置。
