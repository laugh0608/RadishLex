...
5. `M5-P05`：完成安装、升级、修复、默认移除、rollback、数据保留和发行载体。首个载体固定为 Debian 13 ARM64 的系统级本地 `.deb`；P05A 已完成 metadata/rootfs、双构建身份与真实 ARM64 载荷门禁。P05B 已完成确定性载体、实际 `.deb` 流式关系校验、恢复型 receipt/advisory guard、固定系统 observer/executor、concrete mutable port、`/proc` 静止检查、opaque authorized CLI、startup dependency 连接与 fake command/crash matrix；L6 format v1、compile-isolated acceptance controller 与 canonical 脱敏 envelope 也已完成。前三次真实推进依次暴露 Debian 默认配置、`/run/lock` 与 revision profile 缺口；第四套又在 operation ID/CLI 前暴露 source chain 不连续并零 mutation 停止。第五套 chain-continuous ARM64 handoff 与独立 S2 clone 已真实执行一次 upgrade：target 安装后 validation 失败，事务自动恢复 source 并形成 terminal `rolled_back`。离线回归已确认 startup installed-product 层仍固定 revision `1` 并补齐共享 canonical release 门禁；下一步从修复后的 clean commit 重建独立 pair/clone，再逐次授权剩余矩阵。v1 不携带 RadishLex 自有 maintainer scripts，P05C 最后进入独立 guest 授权实机。
