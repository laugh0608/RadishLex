...
- P05A载荷门禁与P05B载体、恢复事务、production port/CLI、startup、L6 controller/pair已完成；六类operation各有独立证据。第三台clean clone已闭合首个crash并冻结source terminal。`install_artifacts_staged`合同已闭合，但既有clone、v2 clone与新v3 clone均在host失败关闭且未进入guest。v7完整诊断manifest `206aa335…7b56c`确认唯一start AppleEvent由UTM接收后触发AppKit主窗口断言，未见对应reply或QEMU事件且零mutation；foreground launch transport v2已通过repository-only合成门禁但未实机。新v4 clone `50B75F88…8038`已以clone/prepared manifest `f76d1943…ff6e2`/`c40c55d9…144b`冻结为DependencyFrozen clean磁盘、stopped target；下一步先闭合prepared绑定与host零进程门，再分批授权唯一foreground start，不得原地启动旧target或运行maintenance/`dpkg`/产品进程。
