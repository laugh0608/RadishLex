...
- P05A载荷门禁与P05B载体、恢复事务、production port/CLI、startup、L6 controller/pair已完成；六类operation各有独立证据，首个crash已闭合，第二个crash已建立launch、断网、canonical input、负向预检与checkpoint的一次性失败关闭控制。当前case、冻结资产、精确manifest、后续顺位与逐步授权只以[`docs/status/current.md`](status/current.md)和[L6 runbook](runbooks/linux-l6-package-matrix.md)为准；不得据本Boundary原地启动旧target、运行maintenance/`dpkg`或越过当前停止线。
