...
当前真实缺口不在输入 ABI、addon 编译、Manager privacy、删除恢复、导入导出、重启矩阵、P05A 产品载荷，或 P05B 的 package relationship/system port/startup gate/controller/pair verifier。L6 format v1、compile-isolated acceptance checkpoint/evidence controller 与双 clean-root ARM64 pair 构建合同已完成；新的 ARM64 pair 仍未实际生成，真实 process/dpkg/crash/font/XDG 证据与 P05C 也尚未完成，这些能力不需要增加平台私有输入 ABI。
