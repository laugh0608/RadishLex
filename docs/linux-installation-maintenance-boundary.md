...
截至 2026-08-15，M5-P05A 已完成 metadata/rootfs、双 addon 构建身份与真实 Debian 13.6 ARM64 载荷门禁。P05B 已完成确定性 `.deb`、actual package streaming relationship、恢复型 receipt/advisory guard、fixed-path observer/executor、concrete mutable `DpkgTransactionPort`、`/proc` quiescence、opaque authorized CLI、startup dependency 连接与 fake command/crash matrix。第四套确认 source chain 不连续；第五套 upgrade 自动恢复 source并形成 `rolled_back`。共享 canonical release 修复后的第六套 upgrade 形成 target `completed` 与冻结 S3；首次 repair 在 staged preflight 因 system port 未把 target-only staging 投影为 effective source而 `aborted_preserved`，未进入 dpkg且 XDG 零漂移。源码与直接 system-port 回归已修复；maintenance-only refresh v1 现已在仓库内闭合冻结 target artifact 与较新 production maintenance ELF 的组合，但 ARM64 handoff和新 S3 clone尚未形成。第五套与该 repair clone仍只作取证，真实 repair重试、acceptance controller 实机与 P05C 继续关闭。
...
P05A 只证明 committed 产品输入能形成 Debian 目标布局。P05B 已完成确定性 `.deb`、actual package/manifest/dependency/version/status relationship、恢复事务、production system port/host、共用只读 startup gate、L6 format v1 与 compile-isolated checkpoint/evidence controller。前两套 pre-receipt 失败关闭；第三套完成 source install/S1/S2；第四套暴露 chain mismatch；第五套 upgrade 自动恢复 source并形成 `rolled_back`，均未污染 XDG。共享 canonical release validator、prior-terminal source anchor、target-only builder 与第六套 handoff已闭合；第六套 upgrade形成 target `completed` terminal与冻结 S3。首次 repair只调用一次旧 production ELF并在 `artifacts_staged` 以 `version_relation_invalid` 进入 `aborted_preserved`；dpkg status/log未变，startup/XDG通过。源码现让 staged repair和host prepare一致地使用target作为effective source，并以production system-port测试覆盖target-only physical staging。package-preserving maintenance-only refresh v1 合同、verifier、builder 源码边界与合成门禁也已闭合；下一顺位须另行授权隔离 ARM64 builder形成新handoff，随后才从S3新建独立clone并另行授权真实repair，再逐项准备process lifecycle、完整crash/retry、显式rollback、外部package lifecycle与数据保留。P05C仍在另一独立guest授权实机，不复用或清理P04/L6现场。
