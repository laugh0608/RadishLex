...
新的repository-only `l6_utm_launch_transport_v2.py`、通用bindings与prepared bindings模块只承载全新target的启动transport，不替换clone或物化事务。控制强绑定clean head、v7十项manifest及20台all-stopped身份、prepared七项manifest `c40c55d9…144b`完整语义、唯一v4 UUID/name与authoritative package路径、UTM bundle `com.utmapp.UTM` 4.7.5 build 118、Info/sdef/App Intent和committed AppleScript identity；live清单只能是v7冻结20台加该唯一stopped target。它先确认UTM/utmctl/QEMUHelper/QEMULauncher/qemu相关进程为0，再以`O_NOFOLLOW` descriptor重算live config/EFI/qcow2并要求owner、mode、single-link、inode、size与prepared SHA-256相符；只读调用plain list/status后重复target校验，两次身份逐字段一致，随后最终进程零门通过才可进入transport。唯一mutation命令是`/usr/bin/osascript`执行固定的先`activate`后UUID start，明确不调用plain `utmctl start`或`--hide`。命令结果、逐次status、terminal 21台清单与backend进程交叉决定0/10/11/12，任何终态都不自动quit/stop/retry/delete或进入guest。16项合成回归通过，但没有真实调用osascript，因此只证明transport合同而不证明窗口故障已经修复。新v4 clone `50B75F88…8038`仍冻结为DependencyFrozen clean磁盘与二十一台all-stopped；prepared后现场有驻留UTM app而无backend，故下一系统批次须先单独授权关闭该app并让相关进程门归零，唯一start仍需另一份授权。
...
P05A 只证明 committed 产品输入能形成 Debian 目标布局。P05B 已完成确定性 `.deb`、actual relationship、恢复事务、production system port/host、共用startup gate、L6 format与controller；六类operation已有独立证据且XDG零漂移。首个crash已闭合。`install_artifacts_staged`由typed guest合同、首次安装回归与单次host start/clone控制固定，所有既有start均在host失败关闭。v7已完整闭合并把失败机制定位到start AppleEvent后的UTM AppKit主窗口断言，仍未进入guest或QEMU；foreground launch transport v2现已绑定prepared证据与live target双重身份并通过16项repository-only门禁，但未实机。新v4独立clone与DependencyFrozen prepared均已闭合，下一步只为该v4分批授权关闭驻留UTM app及唯一start；旧target、`--hide`/任意GUI试跑仍关闭。其他crash、连续完整L6、process/external lifecycle与P05C继续关闭。
