...
P05A 只证明 committed 产品输入能形成 Debian 目标布局。P05B 已完成确定性 `.deb`、actual relationship、恢复事务、production system port/host、共用startup gate、L6 format与controller；六类operation已有独立证据且XDG零漂移。首个crash已闭合。`install_artifacts_staged`由typed guest合同、首次安装回归与单次host start/clone控制固定，所有clone/start均在host失败关闭。前三次诊断停在进程阶段，v4/v5停在日志捕获与空分类门，v6停在`finished`尾记录门。诊断控制v7已绑定全部前序证据，只新增精确整数`1`尾标记与无正文结构摘要，并保持8MiB捕获及事件/脱敏总量门；任何真实v7诊断及后续clone/start仍另行授权。其他crash、连续完整L6、process/external lifecycle与P05C继续关闭。
