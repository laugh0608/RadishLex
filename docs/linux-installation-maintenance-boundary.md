...
截至 2026-08-15，M5-P05A 已完成 metadata/rootfs、双 addon 构建身份与真实 Debian 13.6 ARM64 载荷门禁。P05B 已完成确定性 `.deb`、actual package streaming relationship、恢复型 receipt/advisory guard、fixed-path observer/executor、concrete mutable `DpkgTransactionPort`、`/proc` quiescence、opaque authorized CLI、startup dependency 连接与 fake command/crash matrix。第四套确认source chain不连续；第五套upgrade为`rolled_back`，第六套upgrade形成target `completed`与S3。两台旧repair clone分别冻结staged verifier失败与healthy product completed-noop。`698fe1f`已修复首次健康repair短路，`823afca` ARM64 refresh handoff绑定冻结target package/evidence与新production ELF；全新S3 clone已完成一次真实`repair/same_release/completed`且dpkg同版重装、startup和XDG证据闭合。首台rollback clone的运行态断网通过，但完整只读preflight因必需guest结果缺失失败关闭；rollback及后续主序列、acceptance controller实机与P05C继续关闭。
...
后续所有L6只读probe必须把guest-agent传入文件视为不可信mode：先写私有`0700`传输根，再以受控复制/安装归一化为root-owned `0600` single-link regular file，并复验size/hash后使用。正式检查开始前须以`O_EXCL`创建attempt marker；在pair、artifact、package、dependency/font、receipt、startup、XDG、process与postflight前更新固定phase，并在成功或任一失败退出时原子写root-owned `0600` terminal result。宿主必须回拉并逐字节验真attempt/result；结果缺失归类为transport-inconclusive并失败关闭，不得自动修复或复用同一clone。
