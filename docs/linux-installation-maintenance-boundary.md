...
跨重启恢复不得依赖已经消失的 `/run` 测试 secret，也不得把 raw operation ID 送回宿主。控制只能在完整验证持久 receipt、唯一 operation chain、私有 staging、fresh boot、guard/startup、package/dpkg、XDG/process/network 后，从 receipt 在 guest 内存中取得 operation ID；固定 transient run root 必须原先 absent，再以 create-new `root:root 0700`目录和`0600` single-link secret重建。进入 production `resume`前必须再次验证同一 receipt/staging、secret hash、boot与静止条件；只允许一次 resume及一次postflight。production调用一旦开始而terminal证据缺失或漂移，状态必须记为不确定且不得自动retry、cleanup、stop或quit；调用前失败则保留`artifacts_staged`与现场。transient secret只可由后续独立授权的现场收口处理，不能为了重试而自动删除。

跨重启completed判定必须分开两种boot身份：冻结canonical case的`terminal-postflight.boot_id_sha256`来自原mutation preflight，继续绑定checkpoint/prior boot；执行postflight的current fresh boot由外层driver在读取completed artifacts前后分别从`/proc/sys/kernel/random/boot_id`复验。外层不得为了复用旧validator而改写冻结driver的全局boot常量，也不得把接受prior字段写成放宽current boot验证。历史真实交付driver由冻结readback、固定size/SHA-256和manifest绑定，successor driver则由当前clean committed HEAD绑定，两者不得互相替代。

调用后状态不确定且现场已全停时，新的只读状态观察只能重新绑定冻结结果、source/target、全停inventory与同一transaction身份，再在单次启动后的fresh boot上分类。只有receipt、完整terminal postflight、当前package/dpkg、startup、guard、XDG/process/network及prior/current boot合同全部精确一致时才可交付`completed`；只有原`artifacts_staged` receipt/staging/package/dpkg基线、重启后guard absent、MaintenanceRequired startup与系统静止全部精确一致时才可交付`artifacts_staged`。两者必须互斥，任何未知receipt、证据缺失、双回读或身份漂移均为`state-indeterminate`，不得fallback、resume、retry、repair、cleanup或自动stop；终态stop继续是独立授权的系统动作。

宿主静止门不得把所有`qemu-system-*` accounting name直接视为UTM进程：Android Emulator等独立QEMU不属于UTM注册VM。阻塞集合只接受UTM专属`UTM`、`utmctl`、`QEMUHelper`、`QEMULauncher`角色，并将排除的generic QEMU以PID、accounting name、role和uid的脱敏结构写入证据；UTM inventory、target EFI/qcow2句柄、同PID与无活动`utmctl`仍须独立闭合，不能用排除规则绕过。

单次guest只读probe的transport exit 0与marker存在不能替代结果证据。若固定settle后首次result readback仍缺失，原attempt与guest根立即冻结，不得补拉或重跑probe；后续只能用全新host attempt重新绑定source/target、同一live PID/双句柄与无活动`utmctl`，再对既有result做稳定双读并读取既有phase。结果或phase仍缺失、两次payload漂移、live身份漂移均继续失败关闭，且该消歧不得携带inventory/start、push/exec、resume、dpkg mutation、retry、repair、cleanup或自动stop。

已由稳定双结果与completed phase闭合的现场，只能经独立授权的terminal stop控制结束host runtime。控制先递归绑定completed manifest、source/target与旧baseline，再以一次preflight inventory要求仅target started或全部stopped；started分支必须把当前PID和EFI/qcow2双句柄连续绑定后，至多发送一次`utmctl stop UUID --request`，stopped分支不得重复stop。随后只允许有界host process/lsof静止观察、一次terminal inventory、无UTM backend/target句柄与target固定身份复核。清单、命令返回、句柄、backend或后置身份任一不一致都保持`state-indeterminate`；不得使用status/start、guest/file transport、resume/dpkg、retry/repair/cleanup、force/kill或quit补救。
...
从未注册且不可改写的S2建立独立crash target时，不得注册或复制source config，也不得复用已冻结的crash/transaction terminal充当clone来源。必须先以独立授权形成专用registration-only壳及单成员manifest，固定registered name/UUID/package、`Network=[]`、从未进入guest、零句柄与canonical全停inventory；壳体和S2还必须位于同一APFS设备。若UTM create把部分壳固定写入默认storage，该位置不能成为clone source；只能先通过UTM原生UI Move迁入operator asset root，并在Move前后重新绑定stopped UUID、默认/目标路径互斥、config/EFI/qcow2身份、全停inventory和零句柄，外部路径确认后才可单次update并冻结壳体证据。不得用手工移动、SDEF export/import、全局storage修改或未定义CLI参数替代。clone阶段重新绑定clean committed HEAD、case/source/壳体身份和target absent，只允许一次`utmctl clone`，再以target-local incoming和APFS clonefile分别物化S2的EFI/qcow2、逐项hash后原子替换；source config不进入target。clone前拒绝应保持target absent，clone后任一部分物化、identity/config/inventory漂移或命令异常都视为`state-indeterminate`，不得自动delete、retry、rollback或start。
...
取得新clone授权后，实机前按UTM官方CLI把名称参数修正为`utmctl clone UUID --name NAME`并提交`b86d72f`，修正前未发出真实clone。唯一调用绑定clean `b86d72f`、`65160b12…c1859`、十九台canonical all-stopped清单和冻结registered source；exit 0、空stderr、唯一新增UUID `5B19AEF1…7DAB` stopped与精确package共同返回created，manifest `7ce53048…e5b7`逐项通过。随后只对该新package原子换入未改写DependencyFrozen EFI/qcow2；prepared manifest `b065c7ac…32db`固定config/EFI/qcow2 `d04b00e1…1ff4`/`0b797641…1418`/`4967234b…4b18`、`Network=[]`、双重qcow2、source/registration/target零句柄及二十台前后同清单，独立复核再次通过。后续唯一start仍在90秒后timeout，60次status与terminal list均为二十台全stopped；start/failure/postverify manifest `870f56dd…f3a5`/`59c62c14…bba05`/`8e3d5ced…8386`证明没有进入guest且磁盘未变。v7完整诊断manifest `206aa335…7b56c`进一步固定：UTM收到唯一start AppleEvent后约351ms触发`[self canBecomeMainWindow]`断言和`NSInternalInconsistencyException`，没有对应reply或QEMU事件。该clone仍冻结，不得原地start或把`--hide`当成已验证修复。

新的repository-only `l6_utm_launch_transport_v2.py`、通用bindings与prepared bindings模块只承载全新target的启动transport，不替换clone或物化事务。控制强绑定clean head、前序诊断与prepared manifest语义、唯一target与authoritative package路径、UTM bundle identity和committed AppleScript identity；live清单只能在冻结集合上新增该唯一stopped target。它先确认UTM/utmctl/QEMUHelper/QEMULauncher/qemu相关进程为0，再以`O_NOFOLLOW` descriptor重算live config/EFI/qcow2并要求owner、mode、single-link、inode、size与prepared SHA-256相符；只读调用plain list/status后重复target校验，两次身份逐字段一致，随后最终进程零门通过才可进入transport。唯一mutation命令是`/usr/bin/osascript`执行固定的先`activate`后UUID start，明确不调用plain `utmctl start`或`--hide`。命令结果、逐次status、terminal清单与backend进程交叉决定0/10/11/12，任何终态都不自动quit/stop/retry/delete或进入guest；真实执行结果、后续guest证据与当前停止线只在`current`和L6 runbook维护。
