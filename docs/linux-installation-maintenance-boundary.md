...
新的repository-only `l6_utm_launch_transport_v2.py`与bindings模块只承载未来全新target的启动transport，不替换clone或物化事务。控制强绑定clean head、v7十项manifest及20台all-stopped身份、UTM bundle `com.utmapp.UTM` 4.7.5 build 118、Info/sdef/App Intent和committed AppleScript identity；live清单只能精确新增一个UUID/name stopped target。它在任何transport前两次确认UTM/utmctl/QEMUHelper/QEMULauncher/qemu相关进程为0，期间只读调用plain list/status；唯一mutation命令是`/usr/bin/osascript`执行固定的先`activate`后UUID start，明确不调用plain `utmctl start`或`--hide`。命令结果、逐次status、terminal 21台清单与backend进程交叉决定0/10/11/12，任何终态都不自动quit/stop/retry/delete或进入guest。12项合成回归与真实v7/UTM只读绑定通过，但没有真实调用osascript，因此只证明transport合同而不证明窗口故障已经修复；新clone、DependencyFrozen物化与唯一start仍须分别授权。
...
P05A 只证明 committed 产品输入能形成 Debian 目标布局。P05B 已完成确定性 `.deb`、actual relationship、恢复事务、production system port/host、共用startup gate、L6 format与controller；六类operation已有独立证据且XDG零漂移。首个crash已闭合。`install_artifacts_staged`由typed guest合同、首次安装回归与单次host start/clone控制固定，所有clone/start均在host失败关闭。v7已完整闭合并把失败机制定位到start AppleEvent后的UTM AppKit主窗口断言，仍未进入guest或QEMU；foreground launch transport v2现已通过repository-only门禁但未实机。下一步只为全新target分批授权clone、物化与唯一start；旧target、`--hide`/任意GUI试跑仍关闭。其他crash、连续完整L6、process/external lifecycle与P05C继续关闭。
