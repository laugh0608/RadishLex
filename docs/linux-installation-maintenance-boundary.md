...
截至 2026-08-15，M5-P05A 已完成 metadata/rootfs、双 addon 构建身份与真实 Debian 13.6 ARM64 载荷门禁。P05B 已完成确定性 `.deb`、actual package streaming relationship、恢复型 receipt/advisory guard、fixed-path observer/executor、concrete mutable `DpkgTransactionPort`、`/proc` quiescence、opaque authorized CLI、startup dependency 连接与 fake command/crash matrix。第四套确认source chain不连续；第五套upgrade为`rolled_back`，第六套upgrade形成target `completed`与S3。两台旧repair clone分别冻结staged verifier失败与healthy product completed-noop。`698fe1f`已修复首次健康repair短路，`823afca` ARM64 refresh handoff绑定冻结target package/evidence与新production ELF；全新S3 clone已完成一次真实`repair/same_release/completed`且dpkg同版重装、startup和XDG证据闭合。首台rollback preflight因必需guest结果缺失失败关闭，VM后按授权删除而host证据保留；第二台在正式preflight前触发传输根身份断言并失败关闭；第三台独立S3 clone先形成`passed/postflight`只读terminal，再以唯一production调用完成`rollback/target_older/completed`与真实`38-2 → 38-1`降级，source payload、startup、XDG、进程及断网证据闭合。独立remove clone的唯一只读preflight也已形成`passed/postflight` terminal且无mutation；真实remove、后续主序列、acceptance controller实机与P05C继续关闭。
...
P05A 只证明 committed 产品输入能形成 Debian 目标布局。P05B 已完成确定性 `.deb`、actual package/manifest/dependency/version/status relationship、恢复事务、production system port/host、共用只读 startup gate、L6 format v1 与 compile-isolated checkpoint/evidence controller。前两套 pre-receipt 失败关闭；第三套完成source install/S1/S2；第四套暴露chain mismatch；第五套upgrade为`rolled_back`；第六套upgrade形成target `completed`与S3，均未污染XDG。两次旧repair依次暴露target-only effective-source与healthy validation短路；前者早已修复，后者现由`698fe1f`区分首次repair和已有target proof的恢复。直接回归证明首次健康repair消费permit并恰好apply一次，proof-backed retry零重复mutation，Linux product共118项测试与clippy通过；`823afca` ARM64 refresh handoff、全新S3 clone断网preflight和真实同版repair现均已闭合，dpkg重装与XDG零漂移有独立证据。三个repair clone均不复用；首台rollback VM已授权删除并保留失败证据，第二台在传输根身份断言失败后冻结，第三台先形成`passed/postflight`只读terminal再完成唯一真实`rollback/target_older/completed`，dpkg降级与XDG零漂移均有独立证据。独立remove clone `5EA2BAA2…27A2`已形成`passed/postflight`只读terminal，source package/receipt、startup、XDG、进程和断网均通过且无mutation；十六台VM全停，v3/S3未漂移。下一步只另行授权该clone的真实remove operation；之后再逐项推进process lifecycle、crash/retry、reinstall、外部package lifecycle与数据保留。P05C仍在另一独立guest授权实机。
