...
截至 2026-08-15，M5-P05A 已完成 metadata/rootfs、双 addon 构建身份与真实 Debian 13.6 ARM64 载荷门禁。P05B 已完成确定性 `.deb`、actual package streaming relationship、恢复型 receipt/advisory guard、fixed-path observer/executor、concrete mutable `DpkgTransactionPort`、`/proc` quiescence、opaque authorized CLI、startup dependency 连接与 fake command/crash matrix。第四套确认source chain不连续；第五套upgrade为`rolled_back`，第六套upgrade形成target `completed`与S3。首次repair的target-only staging投影缺口已修复；`b891ed1` refresh handoff与新clone随后形成。新clone唯一调用虽返回`completed`，但健康product validation触发提前成功，dpkg未运行，故repair重装合同仍未闭合；startup/XDG零漂移，证据已冻结。下一步先修复首次健康repair短路，acceptance controller实机与P05C继续关闭。
...
P05A 只证明 committed 产品输入能形成 Debian 目标布局。P05B 已完成确定性 `.deb`、actual package/manifest/dependency/version/status relationship、恢复事务、production system port/host、共用只读 startup gate、L6 format v1 与 compile-isolated checkpoint/evidence controller。前两套 pre-receipt 失败关闭；第三套完成source install/S1/S2；第四套暴露chain mismatch；第五套upgrade为`rolled_back`；第六套upgrade形成target `completed`与S3，均未污染XDG。首次repair的target-only effective-source缺口已修复并形成`b891ed1` refresh载体；新clone唯一调用产生`repair/same_release/completed` receipt与target proof，但dpkg status/log/mtime完全未变。根因是`drive_target`把“已安装且product valid”同时当成首次repair和crash retry的成功条件；下一顺位须让首次repair强制单次apply，仅在已有target proof时允许跳过，并补两侧直接回归。该clone不复用；修复后另建refresh handoff与S3 clone，再逐项推进真实repair、process lifecycle、crash/retry、rollback、外部package lifecycle与数据保留。P05C仍在另一独立guest授权实机。
