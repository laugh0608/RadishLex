...
新的repository-only `l6_utm_launch_transport_v2.py`与bindings模块只承载全新target的启动transport，不替换clone或物化事务。控制强绑定clean head、v7十项manifest及20台all-stopped身份、UTM bundle `com.utmapp.UTM` 4.7.5 build 118、Info/sdef/App Intent和committed AppleScript identity；live清单只能精确新增一个UUID/name stopped target。它在任何transport前两次确认UTM/utmctl/QEMUHelper/QEMULauncher/qemu相关进程为0，期间只读调用plain list/status；唯一mutation命令是`/usr/bin/osascript`执行固定的先`activate`后UUID start，明确不调用plain `utmctl start`或`--hide`。命令结果、逐次status、terminal 21台清单与backend进程交叉决定0/10/11/12，任何终态都不自动quit/stop/retry/delete或进入guest。12项合成回归与真实v7/UTM只读绑定通过，但没有真实调用osascript，因此只证明transport合同而不证明窗口故障已经修复。新v4 clone `50B75F88…8038`已以clone/prepared manifest `f76d1943…ff6e2`/`c40c55d9…144b`冻结为DependencyFrozen clean磁盘与二十一台all-stopped；prepared后现场仍有驻留UTM app而无backend，故唯一start除单独授权外还必须先重新绑定prepared身份并让相关进程门归零。
