...
ABI contract v5 保留 v4 已闭合的产品个人化 Rime session、学习上下文、display/engine index、个人化状态和学习处置，并为 manager user-term view 增加本地导入批次关联；`crates/ime-ffi/include/radishlex_input.h` 已通过 C11 与 Objective-C 编译测试。该 manager 字段不改变 Rime adapter 或输入热路径语义。
