...
Rust gate 直接只读解析固定 `/var/lib/dpkg/status`、`/var/lib/radishlex/install-v1` 与产品 manifest/component；terminal receipt 的 source/target staging 必须重新打开 actual `.deb`/evidence，并按 receipt proof、installed control 与完整 hard dependency relationship 复验。Manager component scope 覆盖完整 bundle tree；Fcitx scope 覆盖 addon、FFI、RimeData 与两份 metadata，并验证 Manager/Fcitx 两份 FFI equivalence、ABI 与 data contract。该 scope 不运行 fontconfig family/glyph probe，也不冒充 L6 全系统证据。gate 不得执行 `dpkg`、创建/修改 state、删除 guard/tmp/receipt、解析 XDG 或打开 userdb/settings/privacy/Rime。development build 只有在 system state/receipt 均不存在且 package 严格 `NotInstalled` 时接受 `AllowedDevelopment + DevelopmentStateAbsent`；product build 只有 terminal installed artifact、Installed package/version/architecture、dependency 与上述 scope 全部一致时接受 `AllowedProduct + InstalledReceiptVerified`。

active/异常 guard、临时或非终态 receipt、Config-Files/半配置/未知 package state、completed remove、product receipt 缺失、development/system 身份交叉、任何 identity drift、未知 result/version/value 或非 `Ok` status 都阻止业务初始化。platform binding 不能自行恢复、降级到 staged profile、创建状态目录或清理现场；恢复只能由另行授权的 opaque privileged command 进入。真实 ARM64 release pair 已冻结 startup-enabled payload 身份；该调用顺序、精确 sibling origin 与只读副作用目前仍只由仓库合成状态和 C++ contract 证明，尚无 package-installed startup 动态证据。
