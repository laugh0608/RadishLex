...
冻结资产宿主根为 `/Users/luobo/VirtualMachines`；第四、第五、第六套、三台 repair clone、第三台rollback、独立remove/reinstall、旧pair真实checkpoint、第三台clean retry source terminal及第二个case clone位于 UTM 默认 Documents 目录。第五套为 `rolled_back` failure/recovery 现场；第六套已形成 target `completed` terminal与未改写 S3；其余terminal与失败现场均不得复用。两台rollback失败clone、旧network失败clone与两台新pair harness失败clone已在持久归档后按授权删除，只保留host evidence。plain `utmctl` 枚举十九台且全部stopped；第三台clean retry已冻结为source terminal，不重启或复用；第二个case仅完成clone身份冻结，未启动或进入guest。`Debian13-ARM64-DependencyFrozen.utm` 故意未注册并继续作为只读 COW 来源。全部 failure/mismatch/rolled-back/terminal/crash clone、handoff、snapshot与host evidence只作各自真相源，不原地重试、清理或混用。UTM只使用`PATH`中的plain `utmctl`，任何时刻最多运行一台VM。
...
| `UTM Documents/RadishLex-Debian13-ARM64-L6-d75818f-crash-install-artifacts-staged.utm` | 已注册；第二个case clone-prepared stopped | UUID `B0B826F6-D7D3-433C-8987-E2D6993A87B3`；只经一次plain clone从reinstall注册壳建立并换入未改写DependencyFrozen磁盘，config/EFI/qcow2为`038274cb…af32`/`0b797641…418`/`4967234b…b18`且`Network=[]`。当前未启动、未传input、未生成operation ID；须另行授权后只启动该VM并先固定运行态断网 |
