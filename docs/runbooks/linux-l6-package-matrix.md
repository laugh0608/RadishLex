...
`l6_utm_launch_transport_v2.py`与独立bindings模块固定以下合同：三项显式授权；clean committed head；主控制/binding/AppleScript regular single-link identity；精确v7 manifest `206aa335…7b56c`的十项、20台all-stopped清单、4,040事件、唯一start send/receive、AppKit断言和零QEMU语义；本机UTM bundle ID、4.7.5 build 118、Info.plist、`UTM.sdef`中`UTMvstar`/saving/recovery及App Intent metadata。live inventory只能是v7原20台身份全部stopped加唯一新UUID/name stopped，故冻结`5B19AEF1…7DAB`不能复用；expected count为21。transport前两次固定`ucomm`观察都必须没有UTM、utmctl、QEMUHelper、QEMULauncher或qemu相关进程，期间只允许plain list/status；随后且仅随后调用一次`/usr/bin/osascript committed-script UUID`。
