...
冻结资产宿主根为 `/Users/luobo/VirtualMachines`；第四、第五和第六套 clone package 位于 UTM 默认 Documents 目录。第五套已形成 terminal `rolled_back` failure/recovery 现场；第六套只完成 handoff/input-switch/只读 preflight，尚未创建 operation 或运行 transaction。plain `utmctl` 枚举十台且全部 stopped。`Debian13-ARM64-DependencyFrozen.utm` 故意未注册并继续作为只读 COW 来源。前五个 L6 failure/mismatch/rolled-back 现场均只作取证，第六套 clone 只允许在后续独立授权中继续；六套 handoff、S0/S1/S2、WAL-drift snapshot 与 host evidence 继续保留且不得混用。UTM 只使用 `PATH` 中的 plain `utmctl`，任何时刻最多运行一台 VM。
...
| `UTM Documents/RadishLex-Debian13-ARM64-L6-80e49ce.utm` | 已注册；第六套 preflight stopped | UUID `193179D5-2595-4628-A063-9EFB73F8EC05`；第六套 handoff 的断网只读 preflight 已通过，未生成 operation ID 或运行 transaction；config/EFI/qcow2 `00bac87d…456`/`7ba7b6bd…ab17`/`0c0c4aea…8d5f`。只允许在另行授权后执行一次 upgrade |
