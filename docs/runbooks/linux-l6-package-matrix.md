...
冻结资产宿主根为 `/Users/luobo/VirtualMachines`；第四、第五、第六套与两台 repair clone package 位于 UTM 默认 Documents 目录。第五套为 `rolled_back` failure/recovery 现场；第六套已形成 target `completed` terminal与未改写 S3；旧 repair clone是 staged-preflight `aborted_preserved` 现场，新 `b891ed1` repair clone是`completed_without_package_reapply`现场。plain `utmctl` 枚举十二台且全部 stopped。`Debian13-ARM64-DependencyFrozen.utm` 故意未注册并继续作为只读 COW 来源。前五个 failure/mismatch/rolled-back 现场与两台 repair clone只作取证；第六套原现场、S3和这些失败现场均不直接继续 mutation。下一次真实repair只能在源码修复、新refresh handoff与全新S3 clone后另行授权。全部 handoff、snapshot 与host evidence继续保留且不得混用。UTM只使用`PATH`中的plain `utmctl`，任何时刻最多运行一台VM。
