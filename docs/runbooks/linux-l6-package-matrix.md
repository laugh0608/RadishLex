...
- 根因是 production `validate_staged_operation` 对 repair 直接使用物理 staged source；而 repair 按合同只 stage target，host prepare 已把 target 同时作为 effective source。源码现统一该投影并增加 single-target system-port 回归。失败 clone 不 resume、重试或复用；release-pair v1 继续要求 executable commit 等于 target release commit并同批构建package。maintenance-only refresh v1 已另行闭合“冻结target package/evidence + 较新production ELF”的可信组合；commit `b891ed1` 的 ARM64 handoff record/ELF `4b41d1c0…fb2cd`/`9a657510…54585` 已冻结，旧 pair/package 未改写。新 clone `BE3579E0-B150-438D-ABE0-53A8D46137F8` 已从未改写 S3 建立、切入 refresh handoff并完成断网只读 preflight；下一步只能另行授权在该 stopped clone 中执行一次真实 repair。

### 当前本地资产登记（非发布证据）

冻结资产宿主根为 `/Users/luobo/VirtualMachines`；第四、第五、第六套与两台 repair clone package 位于 UTM 默认 Documents 目录。第五套为 `rolled_back` failure/recovery 现场；第六套已形成 target `completed` terminal与未改写 S3；旧 repair clone是 staged-preflight `aborted_preserved` 现场，新 `b891ed1` repair clone则只完成 refresh input switch与只读preflight。plain `utmctl` 枚举十二台且全部 stopped。`Debian13-ARM64-DependencyFrozen.utm` 故意未注册并继续作为只读 COW 来源。前五个 failure/mismatch/rolled-back 现场与旧 repair clone只作取证；第六套原现场、S3 和旧失败clone均不直接继续 mutation。新 clone是下一次真实 repair 的唯一候选，但必须另行单步授权。全部 handoff、snapshot 与host evidence继续保留且不得混用。UTM只使用`PATH`中的plain `utmctl`，任何时刻最多运行一台VM。
...
| `RadishLex-L6-Maintenance-Refresh-b891ed1` | 非 VM；package-preserving refresh handoff | record `4b41d1c0…fb2cd`、production maintenance ELF `9a657510…54585`；精确复用 target package/evidence `b211d940…d09c`/`2a1132c6…0e1b`，builder/宿主 verifier 与 mode/link/hash 通过，local evidence `a495e6a2…1cbb`；未重建 package或 acceptance executable，不覆盖、替换或混入旧 pair |
...
| `UTM Documents/RadishLex-Debian13-ARM64-L6-b891ed1-repair.utm` | 已注册；refresh repair preflight stopped | UUID `BE3579E0-B150-438D-ABE0-53A8D46137F8`；从未改写 S3 建立，`Network=[]`，已原子切入 production-only refresh input并通过只读preflight；关机后config/EFI/qcow2 `b06691d1…0ea9`/`a73a3266…9155`/`6267d82e…89d`。尚无operation ID、maintenance invocation或dpkg mutation，是另行授权真实repair的唯一候选 |
