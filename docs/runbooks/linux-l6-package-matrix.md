...
| `UTM Documents/RadishLex-Debian13-ARM64-L6-80e49ce-remove.utm` | 已注册；remove readonly preflight passed stopped | UUID `5EA2BAA2-B9A1-46CC-B496-B37826DD27A2`；从rollback source terminal建立，`Network=[]`，正式只读preflight唯一terminal为`passed/postflight`，未生成新operation ID或执行remove。关机后config/EFI/qcow2 `92568d22…626a`/`4b6292f0…d726`/`91db6f02…6bab`，qcow2复算一致且零句柄；只作下一次另行授权实际remove的连续候选，不用于其他矩阵 |
