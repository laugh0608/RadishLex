...
- 新pair首台retry `45D8205F…F2B4`在input switch前暴露host inventory/path缺陷并已stopped。第二台`0BB24317…307C`完成断网input switch，但case把fresh absent误写成remove-terminal startup预期；Manager/Fcitx实际`0:1:4:15:0`为正确`ReceiptMissing`，故preflight失败关闭。manifest `66212f2c…a8a3e`证明mutation preflight、operation ID、state、checkpoint、guard与transaction均absent；关机freeze `65157b95…cb2b`固定磁盘与零句柄，二十一台全部stopped，两台均不原地重试。

### 当前本地资产登记（非发布证据）

冻结资产宿主根为 `/Users/luobo/VirtualMachines`；第四、第五、第六套、三台 repair clone、第二/第三台rollback clone、独立remove/reinstall及首个crash各clone位于 UTM 默认 Documents 目录。第五套为 `rolled_back` failure/recovery 现场；第六套已形成 target `completed` terminal与未改写 S3；其余terminal与失败现场均不得复用。首台rollback package已按授权删除，只保留host证据。plain `utmctl` 枚举二十一台且全部stopped。`Debian13-ARM64-DependencyFrozen.utm` 故意未注册并继续作为只读 COW 来源。全部 failure/mismatch/rolled-back/terminal/crash clone、handoff、snapshot与host evidence只作各自真相源，不原地重试、清理或混用。UTM只使用`PATH`中的plain `utmctl`，任何时刻最多运行一台VM。
...
| `UTM Documents/RadishLex-Debian13-ARM64-L6-d75818f-crash-install-prepared.utm` | 已注册；input harness failed stopped | UUID `45D8205F-64B5-4E20-A129-895A1A4CF2B4`；canonical inventory顺序与build-environment路径错误导致input switch前失败，未preflight、operation ID或transaction。关机前config/EFI/qcow2为`dcf723b8…84b`/`f762ee52…e76`/`c80cb26c…15d`；只作失败取证，不启动、修补、补证或复用 |
| `UTM Documents/RadishLex-Debian13-ARM64-L6-d75818f-crash-install-prepared-v2.utm` | 已注册；preflight harness failed stopped | UUID `0BB24317-5FC7-4DD3-8572-F0AC282B307C`；断网input switch通过，preflight因fresh absent错误沿用remove-terminal `RemovedProgram/Completed`预期而失败，实际`ReceiptMissing/no receipt`正确。manifest/freeze `66212f2c…a8a3e`/`65157b95…cb2b`证明无mutation preflight、operation ID、state、checkpoint、guard或transaction，磁盘稳定且零句柄；不原地重试或复用，删除前须持久归档临时证据并另行授权 |
