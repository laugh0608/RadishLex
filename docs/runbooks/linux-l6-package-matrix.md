...
- 新pair首台retry `45D8205F…F2B4`在input switch前暴露host inventory/path缺陷。第二台`0BB24317…307C`完成断网input switch，但case把fresh absent误写成remove-terminal startup预期；Manager/Fcitx实际`0:1:4:15:0`为正确`ReceiptMissing`，故preflight失败关闭。两套证据持久归档manifest为`ae4d1da2…ab8d`/`d99cbfad…324c`；两台package随后与旧network/rollback transfer失败clone一起按授权删除。离线guest-case合同提交后，第三台retry `3EC83EB9…593B9`完成clone、断网、canonical input与fresh-absent双preflight，host manifest `0b703d10…5027`证明operation ID/state/checkpoint/guard仍absent。
- 后续独立授权只生成一次operation ID，宿主仅存hash `55af5046…1828`，并只调用一次controller。checkpoint `3fd5df67…4a70`证明prepared通知后完整process group经SIGKILL清零、无dpkg child；crash-state `e18bde6c…8719`固定receipt `install/not_applicable/prepared/chain-1`、guard有效已解锁、status/log未变及Manager/Fcitx `MaintenanceRequired/ActiveGuard`。73项持久manifest `a36b0cbc…8396`通过；当前未resume或执行dpkg mutation。

### 当前本地资产登记（非发布证据）

冻结资产宿主根为 `/Users/luobo/VirtualMachines`；第四、第五、第六套、三台 repair clone、第三台rollback、独立remove/reinstall、旧pair真实checkpoint及第三台clean retry clone位于 UTM 默认 Documents 目录。第五套为 `rolled_back` failure/recovery 现场；第六套已形成 target `completed` terminal与未改写 S3；其余terminal与失败现场均不得复用。两台rollback失败clone、旧network失败clone与两台新pair harness失败clone已在持久归档后按授权删除，只保留host evidence。plain `utmctl` 枚举十八台，仅第三台clean retry started且保持断网/prepared，其余十七台stopped。`Debian13-ARM64-DependencyFrozen.utm` 故意未注册并继续作为只读 COW 来源。全部 failure/mismatch/rolled-back/terminal/crash clone、handoff、snapshot与host evidence只作各自真相源，不原地重试、清理或混用。UTM只使用`PATH`中的plain `utmctl`，任何时刻最多运行一台VM。
...
| `UTM Documents/RadishLex-Debian13-ARM64-L6-d75818f-crash-install-prepared-v3.utm` | 已注册；clean retry prepared started | UUID `3EC83EB9-094B-492B-9756-D47E61C593B9`；从未改写DependencyFrozen盘创建，启动前config/EFI/qcow2为`62040cc9…eb9`/`0b797641…418`/`4967234b…b18`且`Network=[]`。当前唯一started并保持断网；single controller checkpoint已通过，receipt停在prepared且尚未dpkg mutation。exact resume与停止均须另行授权 |
...
首个实机case尚未形成retry terminal。旧pair的`prepared` checkpoint有效，但startup读取合法stale guard时暴露共享父目录策略漂移并在resume前停止；修复pair两台retry又分别暴露input harness与fresh-absent预期缺陷，三台旧现场均不得改写或继续。第三台clean retry `3EC83EB9…593B9`现为同一断网boot中的唯一started VM，canonical input与双preflight、单次`install_prepared` checkpoint均已通过：process group清零、无dpkg child、receipt prepared、startup `ActiveGuard`。下一步须独立授权exact resume到completed及terminal postflight/archive，不能直接进入下一checkpoint。
...
前三个 L6 分别处于 dpkg config、guard parent 与 target manifest profile 停止线；第四个为 artifact-chain mismatch；第五个为`rolled_back`，第六套形成target `completed`与S3。独立clone又闭合真实repair、rollback、remove和reinstall，六类operation已有分散证据但不是同一session。旧pair首个`install_prepared`暴露startup `01777`策略漂移；修复pair两台retry又在transaction前暴露harness缺陷，四台低价值失败clone已归档清理。第三台clean clone `3EC83EB9…593B9`现为唯一started且断网；同一boot已闭合canonical input、fresh-absent双preflight与修复后prepared checkpoint。manifest `a36b0cbc…8396`证明single invocation、process group清零、无dpkg child、prepared receipt、status/log未变、`ActiveGuard` startup与XDG/进程静止。下一步仅在独立授权下exact resume并做terminal postflight/archive；停止、其他case与进一步清理仍关闭。
