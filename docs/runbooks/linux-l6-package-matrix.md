...
- 正常request关机后第三台config/EFI/qcow2为`6295c4ed…29ae`/`a73a3266…9155`/`34733d78…3b10`，qcow2复算一致；host manifest `0b71c24b…3073`覆盖其余26项。S3未漂移、两盘零句柄，十五台VM全停。该clone只在后续独立rollback operation授权下作为连续候选现场，不用于其他矩阵。

### 当前本地资产登记（非发布证据）

冻结资产宿主根为 `/Users/luobo/VirtualMachines`；第四、第五、第六套、三台 repair clone与第二/第三台rollback clone package 位于 UTM 默认 Documents 目录。第五套为 `rolled_back` failure/recovery 现场；第六套已形成 target `completed` terminal与未改写 S3；三台 repair clone依次冻结为 staged-preflight `aborted_preserved`、`completed_without_package_reapply`与真实`completed`现场，第二台rollback clone冻结为transfer-setup failed-closed现场，均不得复用。第三台rollback clone已冻结`passed/postflight`只读资格，只在后续独立rollback operation授权下作为连续候选现场。首台rollback package已按授权删除，只保留host证据。plain `utmctl` 枚举十五台且全部 stopped。`Debian13-ARM64-DependencyFrozen.utm` 故意未注册并继续作为只读 COW 来源。前五个 failure/mismatch/rolled-back 现场、三台 repair clone与第二台rollback clone只作取证；第六套原现场和S3继续只作真相源。全部 handoff、snapshot 与host evidence继续保留且不得混用。UTM只使用`PATH`中的plain `utmctl`，任何时刻最多运行一台VM。
...
| `UTM Documents/RadishLex-Debian13-ARM64-L6-80e49ce-rollback-v3.utm` | 已注册；readonly preflight passed stopped | UUID `EFD15599-7177-4D55-BF17-173EE1F0BBDD`；唯一正式preflight terminal为`passed/postflight`，package/receipt、依赖/字体、startup、XDG、进程和断网全部通过且无mutation。关机后config/EFI/qcow2 `6295c4ed…29ae`/`a73a3266…9155`/`34733d78…3b10`，qcow2复算一致且零句柄；只在后续明确rollback operation授权下继续，不清理或用于其他矩阵 |
