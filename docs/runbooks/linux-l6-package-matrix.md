...
冻结资产宿主根为 `/Users/luobo/VirtualMachines`；第四、第五、第六套与三台 repair clone package 位于 UTM 默认 Documents 目录。第五套为 `rolled_back` failure/recovery 现场；第六套已形成 target `completed` terminal与未改写 S3；前两台 repair clone分别是 staged-preflight `aborted_preserved` 与`completed_without_package_reapply`现场。第三台`823afca` clone只完成断网只读preflight，是下一次真实repair的唯一候选。plain `utmctl` 枚举十三台且全部 stopped。`Debian13-ARM64-DependencyFrozen.utm` 故意未注册并继续作为只读 COW 来源。前五个 failure/mismatch/rolled-back 现场与两台旧 repair clone只作取证；第六套原现场和S3继续只作真相源。全部 handoff、snapshot 与host evidence继续保留且不得混用。UTM只使用`PATH`中的plain `utmctl`，任何时刻最多运行一台VM。
...
| `UTM Documents/RadishLex-Debian13-ARM64-L6-823afca-repair.utm` | 已注册；当前repair候选 stopped | UUID `394217A7-BFC9-43C8-94E6-539FF3F2B6FB`；从S3建立并只完成`823afca` handoff断网只读preflight，operation仍为2。关机后config/EFI/qcow2 `818d8725…7425`/`a73a3266…9155`/`111aaf09…8a6`，qcow2复算一致且零句柄。只有另一次单步授权可启动并进入首次真实repair |
