...
- repository-only v7额外绑定v6八项manifest与精确尾记录拒绝终态。依据本机`log(1)`只要求末尾记录包含`finished`字段，解析现严格接受布尔`true`或精确整数`1`，拒绝其他值、重复或非末尾marker；解析前写入不含正文的record/marker数量、是否末尾与值类型分类。原有64KiB/8MiB捕获、事件/单行/消息/脱敏总量及正文不持久化合同不变。下一次真实采集须重新单独授权并使用新v7输出根。
