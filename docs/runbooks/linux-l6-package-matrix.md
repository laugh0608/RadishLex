...
| `UTM Documents/RadishLex-Debian13-ARM64-L6-d75818f-crash-install-prepared-v2.utm` | 已注册；preflight harness failed started offline | UUID `0BB24317-5FC7-4DD3-8572-F0AC282B307C`；断网input switch通过，preflight因fresh absent错误沿用remove-terminal `RemovedProgram/Completed`预期而失败，实际`ReceiptMissing/no receipt`正确。manifest `66212f2c…a8a3e`证明无mutation preflight、operation ID、state、checkpoint、guard或transaction；须先授权停止并冻结，不原地重试或复用 |
