...
处理任务时先读当前状态，再按任务选择最少的稳定专题；只有 `current` 明确引用的临时专题才进入日常阅读链。
...
- `AGENTS.md` 与 `CLAUDE.md` 只保存长期协作约束，并保持逐字一致；阶段、日期、提交、证据编号、实机清单和下一步不得写入两者。
...
- 文档改动至少运行 `./scripts/check-docs.sh`、`./scripts/check-text-files.sh` 与 `git diff --check`；阶段性或高风险交付再运行 `./scripts/check-repo.sh`。
