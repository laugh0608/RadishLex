...
2. 已落地：userdb schema v9 持久化 domain state、remote observation、local revision、cycle journal/outbox、可信 public lifecycle cache、wrapped epoch ciphertext cache 和 recovery lifecycle；transaction-scoped apply 使 payload、observation、revision 与对象 cursor 同提交或回滚，已验证 lifecycle state 与独立 lifecycle cursor 同事务替换且拒绝回退 / 分叉。明文 master key/shared secret 不进入 SQLite。
