...
初次登记只获文档整理授权。2026-09-08 项目所有者先批准隔离诊断与版本核验、REV-01 单一学习存储方案及 REV-02 Rust SQLite 依赖升级与回归，随后批准 macOS 候选准备和本机旧记录的精确保留式归档。2026-09-09 另行批准并完成 build 39 首次安装及固定路径 Manager 启动；系统输入源操作、真实输入矩阵和发布仍未执行。本专题不改写既有里程碑退出记录，不把历史证据扩展为未测能力。关闭事项必须关联修复提交、实际验证及剩余限制；全部处理后按文档规则退出日常阅读链。
