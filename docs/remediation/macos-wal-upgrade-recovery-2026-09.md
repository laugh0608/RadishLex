...
本文面向 Installer、数据升级与平台适配维护者，记录 2026-09-09 冻结现场之后的仓库诊断、隔离验证及待批准方案。本文不授权真实恢复，也不提供绕过产品事务的手动 SQL、文件搬移或 receipt 修改命令。当前停止线见 [current](../status/current.md)，真实现场证据见[联合验收入口](../runbooks/macos-rev01-rev02-acceptance.md#build-3940-实机升级暂停2026-09-09)。

## 当前结论

本批获准范围是执行错误呈现修复、WAL 升级/恢复方案与隔离验证。错误呈现已修复，五项合法 SQLite WAL 场景通过；恢复生产入口和长期 WAL 处理尚未实现。冻结 operation `aad9cf8ac2dae71b2b659e96a91ea706` 仍未恢复，本批没有打开真实 SQLite connection、运行真实 helper、点击 Installer 或改变原载体。

建议先为冻结的切换前事务增加显式中止入口，通过既有状态机保留原数据库并恢复 source 39 双程序；之后单独实现面向新 operation 的 WAL 源库准备合同。恢复到 39 仅返回已知基线，其 IMK 上下文缺陷仍在，不能据此关闭 REV-01/REV-02 或完成 build 40 输入验收。
...
本次未扩展 ABI、稳定错误枚举或 receipt 格式。`unknown_driver_result` 明确报告失败，但尚不能在 UI 展示完整的内层 `InterruptedSwitch` 分类；冻结的旧 Installer 不会自动获得修复，也不通过再次点击来补采生产错误。
...
建议在现有 InstallerDriver/Executor 与 macOS composition 层增加显式“中止本次升级并恢复源程序”动作，经版本化 bridge 传递。复用 authoritative home、sealed payload/source identity、外层/数据 guard、真实平台 preflight 和程序恢复 adapter；UI 不接受自报路径、operation、source/target 或 receipt 内容。新增 action 的整数映射和未知值失败关闭须覆盖 Rust/Objective-C 双方，具体 ABI 兼容改动随实施一起更新边界。

只支持外层 upgrade `data_coordinating` 且数据 `candidate_verified`、未开始源库切换的状态；动作必须明确确认。已有已中止或恢复中的同一 operation 只走恢复重放，不再次记录中止。对 `switch_prepared` 或更晚的数据状态、其他 operation、未知现场不提供本动作。本入口尚未实现，当前 Installer 的“继续”不能替代它。
...
实施时必须将这些前置变成共享的受控生产验证路径，并补负向合同，不能仅由 runbook 人工核对。
...
2. 进入现有 `resume_install_data_coordination`：内层中止终态进入 `settle_failed_data`，重新验证恢复静止条件；外层落到 `rollback_required`，调用两个 `restore_program_source` 恢复原 source 程序。
...
生产入口实施批还须补 action 新鲜授权、两个 guard 冲突、所有受控资产漂移、恢复两程序各边界中断、错误投影与 sealed source/target 的真实 adapter 合同。使用全新临时合成根，完成 Rust/Objective-C/native 与完整仓库门禁后，准备并核验独立恢复载体，保留原 build 39/40。只有载体和实际动作可审阅后，才请求绑定冻结 operation 的真实恢复授权。
