...
- `docs/manager-ui-boundary.md` 已固定 Phase 4 管理端职责、数据可见性、同步 UI 状态模型、恢复码 / 设备授权停止线、FFI / bridge 边界和第一批功能顺序；`docs/manager-settings-diagnostics.md` 已固定 settings draft JSON、部署证据来源 allowlist、诊断报告字段索引、脱敏边界和验证入口。管理端第一批实现应优先覆盖本地 userdb 管理、学习状态摘要、rank explain 摘要和 sync preflight 摘要；用户可用同步 UI 必须等待可用平台私钥 backend 和目标部署运行证据。
