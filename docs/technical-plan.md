...
当前下一步仍在同步前置治理内。若进入代码，应先补 `DevicePrivateKeyStore` backend capability / unavailable backend 的 Rust 模型和测试，再按 `docs/sync-server-api-storage.md` 落 Go server metadata、storage 和 API 验证；仍不启动真实客户端上传下载。P1 原始事件、本地审计批次和 FFI 明文 payload 继续不得进入同步路径；现阶段不推进平台壳、Flutter manager 主线，也不启动真实上传下载。
