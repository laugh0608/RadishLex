...
- `docs/manager-ui-boundary.md` 已固定 Phase 4 管理端职责、数据可见性、同步 UI 状态模型、恢复码 / 设备授权停止线、FFI / bridge 边界和第一批功能顺序；`docs/manager-local-acceptance.md` 已固定本地验收范围、Phase 4 退出标准映射、验证入口、隐私检查和真实同步停止线；`docs/manager-sync-entry-boundary.md` 已固定真实同步入口进入 UI / bridge 前的恢复码、设备授权、状态门禁、错误分类、诊断脱敏和测试计划；`docs/manager-settings-diagnostics.md` 已固定 settings draft JSON、部署证据来源 allowlist、诊断报告字段索引、脱敏边界和验证入口。管理端第一批实现已覆盖本地 userdb 管理、词库导入导出、import batches、学习状态摘要、rank explain 摘要、sync preflight 摘要、settings draft、sync gate 草案和脱敏诊断报告；用户可用同步 UI 必须等待可用平台私钥 backend 和目标部署运行证据。
