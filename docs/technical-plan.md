...
typed Debian command 层固定 `/usr/bin/dpkg`、私有 staged argv、清空环境、null stdin、有界诊断、配置与 lifecycle projection，但不执行命令。v1 package 不携带 RadishLex 自有 maintainer scripts；外部 scripts/triggers 不构成 transaction completion。现有共用 startup observer 继续只读 `/var/lib/dpkg/status`、receipt staging 与 component scope；Manager/Fcitx 在业务初始化前通过 additive request/result v1 消费 decision，输入 session/key ABI 仍为 v9。startup 尚未连接完整 dependency relationship，host 也没有 mutation 能力。下一顺位是 production fixed-path observer/executor、concrete mutable port、真实进程静止、privileged host/CLI 与 fake crash-command matrix，之后才进入隔离 Debian ARM64 L6。IBus 仅在 Fcitx5 退出后有明确需求时评估。完整边界见 [Linux Fcitx5 平台边界](linux-fcitx5-boundary.md)、[Linux Manager 本地验收边界](linux-manager-local-acceptance.md) 与 [Linux 安装维护边界](linux-installation-maintenance-boundary.md)。
