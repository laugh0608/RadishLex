...
当前下一步仍在同步服务端前置治理内。若进入代码，应继续按 `docs/sync-server-api-storage.md` 落签名验证抽象、HTTP API 前的错误语义和 metadata API 验证；仍不启动真实客户端上传下载。P1 原始事件、本地审计批次和 FFI 明文 payload 继续不得进入同步路径；现阶段不推进平台壳、Flutter manager 主线，也不启动真实上传下载。
