...
- 平台私钥存储 backend 平台 runbook 和真实 backend 未完成前，不进入管理 UI 同步主线；Go server 当前只能先按专题文档验证 metadata、storage、签名、版本冲突、encrypted object HTTP handler、runtime 装配、脱敏日志、server smoke、Docker Compose 本地 / 部署态入口、容器实际启动 smoke 和错误语义边界。Rust remote client 已验证 DTO / transport trait、std-only `http://` HTTP transport、两客户端 userdb harness、直连 Go server 的短生命周期测试和两客户端真实 Go HTTP 同步；生产平台私钥 backend、认证 / 备份 / 外部 TLS 部署边界没有确认前，不开放给真实用户同步。
