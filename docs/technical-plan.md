...
首个`install_prepared` checkpoint在旧pair上因startup遗漏Debian精确`01777`共享锁父目录合同而于resume/dpkg前失败关闭；共享策略已修复，target `d75818f`的新pair已冻结。两台新pair retry又分别在input switch前和只读preflight内暴露canonical inventory/file-name与fresh-absent startup预期的host harness缺陷，均未进入transaction；其证据已持久归档，低价值package按授权删除。committed离线回归已固定canonical input及fresh-absent/remove-terminal互斥预期；第三台clean absent clone现已在同一断网boot闭合input与fresh-absent preflight，operation ID仍未生成。下一步只在新授权下执行一次有效`install_prepared` checkpoint；其余七个crash/retry、连续完整L6和P05C仍须逐项授权。IBus仅在Fcitx5退出后有明确需求时评估。完整边界见 [Linux Fcitx5 平台边界](linux-fcitx5-boundary.md)、[Linux Manager 本地验收边界](linux-manager-local-acceptance.md)、[Linux 安装维护边界](linux-installation-maintenance-boundary.md) 与 [L6 runbook](runbooks/linux-l6-package-matrix.md)。
