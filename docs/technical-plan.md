...
- `server/sync-server` 已起步 Go module，覆盖配置默认值、API request / error DTO、storage interface、SQLite metadata migration 文本、内存 storage 验证模型和测试；当前只验证服务端可见 metadata、设备状态、版本冲突、密文 hash / 长度和错误语义，不实现 HTTP handler、真实 SQLite driver-backed repository、生产 local object store 或真实远端上传下载。
...
当前下一步仍在同步服务端前置治理内。若进入代码，应继续按 `docs/sync-server-api-storage.md` 落 SQLite-backed metadata repository、local object storage transaction、签名、版本冲突和错误语义验证；仍不启动真实客户端上传下载。P1 原始事件、本地审计批次和 FFI 明文 payload 继续不得进入同步路径；现阶段不推进平台壳、Flutter manager 主线，也不启动真实上传下载。
