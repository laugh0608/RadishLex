...
- `docs/manager-ui-boundary.md` 已固定 Phase 4 管理端职责、数据可见性、同步 UI 状态模型、恢复码 / 设备授权停止线、FFI / bridge 边界和第一批功能顺序；`docs/manager-local-acceptance.md` 已固定本地验收范围、Phase 4 退出标准映射、验证入口、隐私检查和真实同步停止线；`docs/manager-sync-entry-boundary.md` 已固定真实同步入口进入 UI / bridge 前的恢复码、设备授权、状态门禁、错误分类、诊断脱敏和测试计划，并引用部署证据来源；`docs/manager-recovery-device-auth-flow.md` 已固定恢复码 setup / restore、设备 join / revocation 四条只读交互状态机；`docs/manager-settings-diagnostics.md` 已固定 settings draft JSON、部署证据来源 allowlist、连接健康摘要、诊断报告字段索引、脱敏边界和验证入口，明确 settings draft 只保存非敏感 evidence source 标签和净化后的 `sync_connection_health_summary`，不保存证据包正文或探测原文。管理端第一批实现已覆盖本地 userdb 管理、词库导入导出、import batches、学习状态摘要、rank explain 摘要、sync preflight 摘要、settings draft、sync entry gate、连接健康、恢复码 / 设备授权只读准备态、readiness 聚合摘要、future bridge readiness mapper、只读交互进入计划和脱敏诊断报告；用户可用同步 UI 必须等待可用平台私钥 backend、恢复 / 授权实现测试和发布级部署证据。
...
- `apps/radishlex-manager` 已创建 Flutter macOS 起步工程，通过受控 `ManagerBridge` contract 接入管理数据源；默认仍使用合成 fixture，显式配置 `RADISHLEX_MANAGER_DB` 与 `RADISHLEX_MANAGER_FFI_LIBRARY` 时可切到第一批真实 Dart FFI bridge。当前 FFI bridge 覆盖本地 userdb 词条 list / delete、用户词库 inspect / import / export、import batches、learning status 摘要、rank explain 摘要、sync preflight 摘要、非 secret settings JSON 草案持久化和脱敏诊断报告导出；`rank explain` 区域通过专用 `ime-ffi` ABI 读取单候选贡献项，Flutter 只展示复制后的非敏感摘要。设置页已显示配置来源诊断、sync gate 草案预览、部署证据来源标签和连接健康摘要回填，settings JSON 当前为 `format_version: 1` 并覆盖旧 v1 缺字段降级、非法版本 / 非法 URL / 非 allowlist evidence source、非法 `sync_connection_health_summary` 拒绝；诊断报告包含 gate source / stop line、连接健康、readiness 聚合和只读交互进入计划，但不含用户词、文件路径、token、恢复码、签名、wrapped material 或 payload bytes，支持按字段分组预览、筛选、复制与导出一致的脱敏文本。manager UI 结构已拆出跨页 action 编排、词库子组件、学习子组件、同步子组件、设置诊断子组件、页面级 widget tests 和 action helper 回归测试；bridge 失败按操作和分类展示结构化错误码。sync gate 状态由设置草案、隐私模式、平台私钥 backend gate、部署证据来源草案、连接健康摘要和恢复 / 授权只读 readiness 派生；同步启用按钮在非生产状态下保持关闭，不接真实远端、恢复码生成 / 输入、join request 创建、设备授权成功或设备撤销。
...
- [Manager 恢复码与设备授权交互进入条件](manager-recovery-device-auth-flow.md)：恢复码 setup / restore、设备 join / revocation 四条只读交互状态机、readiness 聚合和只读进入计划。
