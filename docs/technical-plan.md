...
M5 先形成 addon/FFI/build contract，再进入真实 Wayland、X11、常见应用、secure/unknown 上下文和同库个人化验收，最后治理 Linux 安装、升级、修复、移除和数据保留。系统 package 事务由独立 `platforms/linux-product/` 承担：它以 root state identity、canonical append-only receipt、Unix socket guard、私有 source/target artifact staging 和 `DpkgTransactionPort` 隔离 Debian package 状态；不复用 macOS 双 bundle rename，不读取用户 XDG，也不进入输入热路径。该层现已提供共用只读 startup observer，直接解析 `/var/lib/dpkg/status` 并复验 receipt staging proof；Manager scope 覆盖完整 bundle tree，Fcitx scope 覆盖 addon/FFI/RimeData/两份 metadata 和双 FFI equivalence。Manager 在 `umask(0077)` 后、Flutter 初始化前，Fcitx factory 在 Engine/input FFI/XDG/Rime 初始化前，通过 additive request/result v1 Linux startup ABI 消费同一 decision；C++ binding 用 `dladdr`/canonical path 把 startup/error/input symbols 绑定到精确 sibling FFI。输入 session/key ABI contract 仍为 v9，`development-staged` 与 `debian-system-product` 由编译身份隔离，host 不取得 mutation 能力。observer 不调用 `dpkg`、不创建 state 或触碰 XDG，其 component scope 不冒充全 package runtime inventory。下一顺位仍是 mutable dpkg adapter、外部依赖/版本关系与 maintainer-script 投影，再进入隔离 Debian ARM64 L6 matrix。IBus 只有在 Fcitx5 退出后存在明确需求时再评估。完整平台职责见 [Linux Fcitx5 平台边界](linux-fcitx5-boundary.md)，Manager host、privacy、同库并发和产品验收见 [Linux Manager 本地验收边界](linux-manager-local-acceptance.md)，系统安装事务见 [Linux 安装维护边界](linux-installation-maintenance-boundary.md)。
