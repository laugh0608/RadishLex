...
- 真实平台私钥存储 backend 的生产可用状态；当前 `apple-keychain-v1` 已 feature-gated 接线，但真实 smoke 阻塞于 `ed25519-v1` 创建，不能作为可用生产 backend；`android-keystore-v1` 已补 runbook，但尚未接 Kotlin / JNI 或 Android gated smoke。
