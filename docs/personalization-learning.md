...
25. 已补 `ime-sync::SyncEnvelopeAssembler`，固定 Rust 内部 P2 payload 到 envelope 的组装边界；在恢复码 KDF、签名 / 设备密钥存储和真实 payload / userdb 写回接线稳定前，不进入远端同步或管理 UI 同步主线。
