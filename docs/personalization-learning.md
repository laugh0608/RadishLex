...
29. 已补签名 / 设备密钥存储 Rust 模型，覆盖 Ed25519 test-memory signer、signed sync object manifest、signed recovery record、signed device authorization 和 signed device revocation；在生产恢复流程、平台私钥存储 backend 和真实 payload / userdb 写回接线稳定前，不进入远端同步或管理 UI 同步主线。
30. 已补真实 userdb P2 payload 解析到 merge input 的接线；在合并结果写回真实 userdb、生产恢复流程和平台私钥存储 backend 稳定前，不进入远端同步或管理 UI 同步主线。
