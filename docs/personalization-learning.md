...
23. 已补 `ime-crypto` / `ime-sync` 的设备、key epoch、device wrapping、recovery material、授权包、撤销记录和对象版本冲突 Rust 模型；在删除合并、恢复码 KDF、签名 / 设备密钥存储和生产组装边界稳定前，不进入远端同步或管理 UI 同步主线。
