...
- production 源码、L6 format v1、acceptance controller、release-pair 与 maintenance-only refresh v1 合同已闭合。六类 operation 均有独立证据。首个 `install_prepared` checkpoint 暴露的 Debian `01777` startup 缺陷已由 `a6622d4` 修复，target `d75818f` 的 pair record `c74fac12…9849` 已冻结。两台新pair重试clone均在transaction前失败关闭：`45D8205F…F2B4`已停止，原因为input harness inventory/path不一致；`0BB24317…307C`完成断网input switch后，case把fresh absent错误预期为remove-terminal `RemovedProgram`，实际正确返回`ReceiptMissing`。manifest `66212f2c…a8a3e`证明无mutation preflight、operation ID、state、checkpoint或guard。二十一台VM中仅第二台离线运行，待授权停止冻结。v1 package不含RadishLex maintainer scripts。
...
3. 第六套及repair/rollback/remove/reinstall terminal均冻结。旧pair的`FD24ADFF…17C056`只执行一次`install_prepared` checkpoint，因startup遗漏合法`01777`父目录而在resume/dpkg前失败，manifest `5d8c914a…3e42a`冻结。修复handoff由record `c74fac12…9849`闭合；新pair首台retry `45D8205F…F2B4`已停止并冻结input harness失败，第二台`0BB24317…307C`的input通过但preflight因case混淆fresh absent与remove terminal而失败，manifest `66212f2c…a8a3e`证明未生成mutation preflight、operation ID或transaction。两台均不得原地修补或复用；下一步须先授权停止第二台，再修正startup预期并另建clean absent clone。连续完整L6、P05C、发布、推送、清理、真实同步及其他平台保持关闭。
