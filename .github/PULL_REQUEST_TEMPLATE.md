...
- 关联 Issue / 任务：
- 目标分支：`dev` / `master`（目标为 `master` 时，请说明本次阶段性收口范围）
- 变更类型：
  - [ ] 功能
  - [ ] 修复
  - [ ] 重构
...
- [ ] 本次改动符合 `docs/status/current.md` 的当前里程碑、批次和停止线，或已明确说明为何需要例外
...
- [ ] 若目标为 `master`，`dev` 已包含上一次 master 合并结果，并已将本 PR 合并后的 `master -> dev` 回同步列为收尾动作
