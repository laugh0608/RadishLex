...
随后第三套 pair 的 source `09ed1228…bec` 已真实安装并形成 terminal S1/S2。第四套证明同版本重建 source 不能替代 terminal artifact；builder 因此改为精确冻结 committed source并只构建 clean target。第五套transaction为`rolled_back`，第六套upgrade形成target `completed`与S3；第三台repair及独立rollback、remove、reinstall均已真实闭合，六类operation各有证据且XDG零漂移。首个`install_prepared`已闭合prepared checkpoint、exact resume terminal与关机冻结；第二个`install_artifacts_staged`已闭合typed合同、checkpoint、fresh-boot只读transaction `completed`、正常停止及结果binding。第三个`upgrade_quiesced`已固定不可变S2、checkpoint/resume合同、repository-only专用registration shell创建/冻结控制和clone前门；首个真实create因AppleScript configuration类型错误安全闭合`failed-closed-absent`，`10cc4e1`已修复但尚未重试，专用壳与真实target仍不存在。精确现场、冻结证据、当前停止线与下一授权只读[`docs/status/current.md`](../../docs/status/current.md)和[L6 runbook](../../docs/runbooks/linux-l6-package-matrix.md)。
