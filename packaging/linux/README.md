...
- `l6-matrix.json` 固定 `debian13-arm64-ephemeral-v1` 的 guest、不同 commit release pair、六步主序列、八个 crash checkpoint、probe/evidence 与逐 mutation 授权；`l6-upgrade-quiesced-case.json`另行固定第三个case的不可变S2、专用registration shell创建/冻结控制、独立clone前门、checkpoint/resume语义与六段授权；二者都不是可执行安装脚本。
- `l6-asset-retirement-allowlist.json`只固定经过人工选择的精确VM批次、UUID、绝对storage path、磁盘身份与既存证据锚点；它不自动发现资产，也不授权prepare或删除。
...
随后第三套 pair 的 source `09ed1228…bec` 已真实安装并形成 terminal S1/S2。第四套证明同版本重建 source不能替代terminal artifact；builder因此精确冻结committed source并只构建clean target。第五套transaction为`rolled_back`，第六套upgrade形成target `completed`与S3；第三台repair及独立rollback、remove、reinstall均已真实闭合，六类operation各有证据且XDG零漂移。首个`install_prepared`已闭合prepared checkpoint、exact resume terminal与关机冻结；第二个`install_artifacts_staged`已闭合typed合同、checkpoint、fresh-boot只读transaction `completed`、正常停止及结果binding。第三个`upgrade_quiesced`已固定不可变S2、checkpoint/resume合同、独立clone前门和六段授权；专用registration shell经UTM原生Move与条件式typed update后以`frozen`闭合，真实target仍不存在。P05B随后固定5台注册锚点预算并启用精确资产退休控制；前三批经独立授权把注册列表从22台降至11台，11个package均absent，三个delete manifest冻结。第四批3台的prepare与delete控制已闭合，真实mutation未授权。精确现场、冻结证据、当前停止线与下一授权只读[`docs/status/current.md`](../../docs/status/current.md)、[L6 runbook](../../docs/runbooks/linux-l6-package-matrix.md)和[资产生命周期账本](../../docs/runbooks/linux-l6-asset-lifecycle.md)。
