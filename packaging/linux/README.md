...
随后第三套 pair 的 source `09ed1228…bec` 已真实安装并形成 terminal S1/S2。第四套证明同版本重建 source 不能替代 terminal artifact；builder 因此改为精确冻结 committed source并只构建 clean target。第五套transaction为`rolled_back`，第六套upgrade形成target `completed`与S3；第三台repair及独立rollback、remove、reinstall均已真实闭合，六类operation各有证据且XDG零漂移。首个`install_prepared`已闭合prepared checkpoint、exact resume terminal与关机冻结；第二个`install_artifacts_staged`已闭合typed合同、首次安装回归以及launch、断网、canonical input、negative preflight和checkpoint的一次性控制。精确现场、冻结证据、当前停止线与下一授权只读[`docs/status/current.md`](../../docs/status/current.md)和[L6 runbook](../../docs/runbooks/linux-l6-package-matrix.md)。
