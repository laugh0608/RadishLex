...
随后第三套 pair 的 source `09ed1228…bec` 已真实安装并形成 terminal S1/S2。第四套证明同版本重建 source 不能替代 terminal artifact；builder 因此改为精确冻结 committed source并只构建 clean target。第五套transaction为`rolled_back`，第六套upgrade形成target `completed`与S3；两台旧repair clone分别冻结staged relation失败与completed-noop。`698fe1f`已修复healthy repair短路，maintenance-refresh v1也只接受包含该提交的clean descendant；`823afca` ARM64 handoff精确复用冻结target package/evidence并形成新production ELF，既有`b891ed1` handoff/clone只作历史取证。第三台repair clone随后完成真实同版重装，独立rollback clone完成`38-2 → 38-1`降级；当前独立remove clone已通过只读preflight，下一步只在新授权下执行实际remove，不能复用旧资产或重建package。
