...
随后第三套 pair 的 source `09ed1228…bec` 已真实安装并形成 terminal S1/S2。第四套证明同版本重建 source 不能替代 terminal artifact；builder 因此改为精确冻结 committed source并只构建 clean target。第五套transaction为`rolled_back`，第六套upgrade形成target `completed`与S3；两台旧repair clone分别冻结staged relation失败与completed-noop。`698fe1f`已修复healthy repair短路，maintenance-refresh v1也只接受包含该提交的clean descendant；`823afca` ARM64 handoff精确复用冻结target package/evidence并形成新production ELF，既有`b891ed1` handoff/clone只作历史取证。第三台repair clone随后完成真实同版重装，独立clone依次完成rollback、默认remove与target reinstall；六类operation已有独立证据，startup与XDG零漂移均通过。首个`install_prepared` crash checkpoint因startup共享锁父目录合同漂移在resume/dpkg前失败关闭；`a6622d4`修复target `d75818f`的新pair已冻结。第三台clean absent clone已逐项闭合断网、canonical input与fresh-absent preflight，下一步须独立授权单次重试该checkpoint；不能复用terminal/旧crash资产、热替换旧pair或重建source package。
