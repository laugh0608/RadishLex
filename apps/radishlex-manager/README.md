...
- 本地 userdb 词条管理视图，包含搜索过滤、空态、导入历史筛选 / 排序 / 批次联动审计、删除确认和操作失败分类提示。
- 本地学习状态摘要。
- `rank explain` 非敏感摘要。
