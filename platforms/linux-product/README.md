...
当前 source install、S1/S2、pre-receipt failure、chain mismatch 与第五套 `rolled_back` recovery 均已取证。第六套upgrade形成target `completed`与S3；两台旧repair clone分别冻结target-only relationship失败与healthy-product completed-noop。`698fe1f`让fresh repair跳过valid-target快捷路径、消费permit并apply一次，只让已有target proof的retry零重复；两侧直接回归与Linux product 118 tests通过。`823afca` ARM64 refresh handoff绑定未改写target package/evidence并形成新production ELF，第三台repair clone已完成真实同版重装；后续独立clone依次形成真实rollback、remove与reinstall terminal，target恢复、startup与XDG零漂移均有严格postflight。六类operation已有独立证据。首个`install_prepared` checkpoint因startup对合法`root:root 01777 /run/lock`误报`GuardInvalid`而在resume/dpkg前失败关闭；`a6622d4`已让store/startup共用权限策略，修复target `d75818f`的新pair已冻结。下一步从新clean absent clone逐项闭合preflight后单次重试该checkpoint，再决定后续crash/retry；证据见L6 runbook，完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
