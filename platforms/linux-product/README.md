...
当前 source install、S1/S2、三类 pre-receipt failure 与第四套 chain mismatch 均已取证；第五套 target `1ebbdab` chain-continuous ARM64 pair/handoff 与独立 S2 clone 只读 preflight 已完成，九台注册 VM 全部停止。旧 ELF/handoff/guest 不热替换或重试，第五套 target package transaction 尚未开始。仓库测试只使用 actual 合成 package、fake executor/observer/backend 与临时目录；真实证据单独记录在 L6 runbook。完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
