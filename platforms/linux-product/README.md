...
- [`packaging/linux/l6-release-pair.json`](../../packaging/linux/l6-release-pair.json) 将 source `55351f2` revision 1 与 target revision 2 绑定为同 data contract 的相邻 pair；`build-linux-l6-release-pair.sh` 从两个独立 clean root 构建并强校验两份 `.deb`，再分别冻结 production `--no-default-features` maintenance ELF 与 acceptance ELF。target `e5b6da1`/`2fa1b8c` 的两个 pair 现均只作 pre-receipt 失败取证；包含 `f0415ad` 的 target `512e8ab` 已形成第三个独立 pair/handoff/guest/S0，record SHA-256 为 `2f2deaed6c8886cfcc4751ccc56439bda95f311767587dc74103645664258697`。

当前 production 代码、fake command/crash matrix、L6 format、compile-isolated checkpoint/evidence controller 与 release-pair builder/verifier 已闭合。两次首次授权 install 分别暴露 Debian 默认配置与 sticky lock parent 缺口，均停于 receipt/package mutation 之前；第三套输入已在独立断网 guest 真实完成 source revision 1 install，terminal receipt、package/inventory、20 项依赖、字体、双 component startup gate 与 XDG absent postflight 通过。两个旧 handoff/S0 和离线 failure overlay 继续保留，第三个 guest 已停止；下一步只能另行授权冻结 S1，再逐项准备 S2、upgrade 与其余 matrix。仓库测试只使用 actual 合成 package、fake executor/observer/backend 与临时目录；真实 install 证据单独记录在 L6 runbook。完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
