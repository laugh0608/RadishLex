...
- [`packaging/linux/l6-release-pair.json`](../../packaging/linux/l6-release-pair.json) 将 source `55351f2` revision 1 与 target revision 2 绑定为同 data contract 的相邻 pair；`build-linux-l6-release-pair.sh` 从两个独立 clean root 构建并强校验两份 `.deb`，再分别冻结 production `--no-default-features` maintenance ELF 与 acceptance ELF。旧 source `55351f2`/target `e5b6da1` pair 只作失败取证；修复后的 source `55351f2`/target `2fa1b8c` pair 已独立复验并由新 handoff/S0 绑定。

当前 production 代码、fake command/crash matrix、L6 format、compile-isolated checkpoint/evidence controller 与 release-pair builder/verifier 已闭合。旧 pair 的首次授权 install 在 `bb84d4a` 修复前停于 receipt 之前；新 pair、独立 handoff/guest 与 S0 已准备，但真实 `/usr/bin/dpkg` mutation、字体 family/glyph/owner、外部 package lifecycle 与 L6 完成证据仍不存在。旧 handoff/S0 和离线 failure overlay 继续保留，下一步须重新单步授权新 guest 的首次 install。仓库测试只使用 actual 合成 package、fake executor/observer/backend 与临时目录；不得据此宣称系统安装。完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
