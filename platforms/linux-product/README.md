...
当前 source install、S1/S2、pre-receipt failure、chain mismatch 与第五套 `rolled_back` recovery 均已取证。第六套upgrade形成target `completed`与S3；两台旧repair clone分别冻结target-only relationship失败与healthy-product completed-noop。`698fe1f`让fresh repair跳过valid-target快捷路径、消费permit并apply一次，只让已有target proof的retry零重复；两侧直接回归与Linux product 118 tests通过。`823afca` ARM64 refresh handoff绑定未改写target package/evidence并形成新production ELF，第三台repair clone已完成真实同版重装；后续独立clone依次完成真实`38-2 → 38-1` rollback与`remove/not_applicable/completed`，package/product tree absent、startup失败关闭且XDG零漂移。独立reinstall clone的absent-terminal只读资格已通过；下一步只在新授权下重新闭合mutation preflight并单次执行真实`reinstall_target`。证据见L6 runbook，完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
