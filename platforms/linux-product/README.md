...
当前 source install、S1/S2、pre-receipt failure、chain mismatch 与第五套 `rolled_back` recovery 均已取证。第六套upgrade形成target `completed`与S3；首次repair的target-only relationship缺口已修复。`b891ed1` refresh新clone唯一调用形成completed receipt，但`drive_target`因installed product valid而直接记录target proof，dpkg status/log/mtime未变，因此真实repair未闭合。下一步先让首次健康repair强制单次apply、已有target proof的retry不重复，再形成新refresh/clone；证据见L6 runbook，完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
