...
当前 production 代码、fake command/crash matrix、L6 format 与 compile-isolated checkpoint/evidence controller 已闭合，但新的 ARM64 release pair、真实 Linux `/proc`、mutable `dpkg`、字体 family/glyph/owner、外部 package lifecycle 和 Debian ARM64 L6 均未执行。仓库测试只使用 actual 合成 package、fake executor/observer/backend 与临时目录；不得据此宣称系统安装。完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
