...
- [`packaging/linux/l6-release-pair.json`](../../packaging/linux/l6-release-pair.json) 将 source `55351f2` revision 1 与 target revision 2 绑定为同 data contract 的相邻 pair；builder 从两个独立 clean root 构建两份 `.deb`，再用 target production Rust verifier 逐侧解析后冻结 maintenance/acceptance ELF。旧 target `e5b6da1`/`2fa1b8c` 暴露环境缺口，`512e8ab` 又在 upgrade 前暴露 revision profile 缺口；三套输入均只作失败取证。

当前 source install、S1/S2 与三类 pre-receipt 失败均已取证，七台注册 VM 全部停止。revision 修复后的 parser 已只读通过第三套真实 pair 两侧，但旧 target ELF/handoff/guest 不热替换或重试；下一步需先重建第四套 pair，再从独立 S2 clone 逐步授权。仓库测试只使用 actual 合成 package、fake executor/observer/backend 与临时目录；真实证据单独记录在 L6 runbook。完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
