...
当前 source install、S1/S2、三类 pre-receipt failure 与第四套 chain mismatch 均已取证，八台注册 VM 全部停止。prior-terminal anchor 与 target-only builder 已通过 repository 门禁，但第五套真实 ARM64 pair/handoff 尚未构建；旧 ELF/handoff/guest 不热替换或重试。仓库测试只使用 actual 合成 package、fake executor/observer/backend 与临时目录；真实证据单独记录在 L6 runbook。完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
