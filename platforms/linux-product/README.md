...
当前 source install、S1/S2、三类 pre-receipt failure 与第四套 chain mismatch 均已取证；第五套 target `1ebbdab` chain-continuous ARM64 pair/handoff 与独立 S2 clone 已执行一次 production upgrade，target 安装后的 validation 失败并自动恢复 source，terminal 为 `rolled_back`、无需人工恢复，九台注册 VM 全部停止。旧 ELF/handoff/guest 不热替换或重试，第五套只作 failure/recovery 取证。仓库测试只使用 actual 合成 package、fake executor/observer/backend 与临时目录；下一步先在宿主离线建立 target-validation 可重复失败并补门禁，真实证据单独记录在 L6 runbook。完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
