...
当前 source install、S1/S2、pre-receipt failure、chain mismatch 与第五套 `rolled_back` recovery 均已取证。第六套 production upgrade形成 target `completed` 与冻结 S3；首次 repair 只调用一次旧 production ELF，在 staged preflight 因 target-only relationship 未投影为 effective source而 `aborted_preserved`，未进入 dpkg。该 production port 缺口与直接回归已修复，十一台 VM 全停；旧 ARM64 ELF、失败 clone和同版本重建 package均不得替代新 handoff。现有release-pair v1不能表达冻结target artifact与较新ELF的组合；下一步先固定maintenance-only refresh合同，再只刷新production maintenance ELF并锚定旧record与既有target artifact，之后才从S3新clone。真实证据单独记录在 L6 runbook，完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
