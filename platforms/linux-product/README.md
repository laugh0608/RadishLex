...
当前 source install、S1/S2、pre-receipt failure、chain mismatch 与第五套 `rolled_back` recovery 均已取证。第六套upgrade形成target `completed`与S3；两台repair clone分别冻结target-only relationship失败与healthy-product completed-noop。`698fe1f`现让fresh repair跳过valid-target快捷路径、消费permit并apply一次，只让已有target proof的retry零重复；两侧直接回归与Linux product 118 tests通过。`823afca` ARM64 refresh handoff已绑定未改写target package/evidence并形成新production ELF；下一步另行从S3建立全新clone并完成断网只读preflight。证据见L6 runbook，完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
