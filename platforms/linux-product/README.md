...
当前 source install、S1/S2、三类 pre-receipt failure 与第四套 chain mismatch 均已取证；第五套 target `1ebbdab` chain-continuous ARM64 pair/handoff 与独立 S2 clone 已执行一次 production upgrade，target 安装后的 validation 失败并自动恢复 source，terminal 为 `rolled_back`、无需人工恢复。离线完整 installed-product 回归已证明 startup manifest 的独立 revision `1` 假设会拒绝 `38-2`，现通过 relationship/startup 共用 canonical release validator、`38-2` 正向和非 canonical 负向门禁闭合。修复 commit `80e49ce` 的第六套 pair/handoff 与独立 S2 clone 只读 preflight 已通过；十台注册 VM 全部停止，没有新 operation ID、maintenance/acceptance CLI 或 dpkg mutation。旧 ELF/handoff/guest 不热替换或重试，第五套只作 failure/recovery 取证；下一步只在另行授权后执行一次第六套 source→target upgrade。真实证据单独记录在 L6 runbook，完整边界见 [`docs/linux-installation-maintenance-boundary.md`](../../docs/linux-installation-maintenance-boundary.md)。
