...
L6 handoff 另由 [`packaging/linux/l6-release-pair.json`](../../packaging/linux/l6-release-pair.json) 与 `build-linux-l6-release-pair.sh` 冻结：source `55351f2` revision 1、target revision 2，target 的 production maintenance binary 必须不含 acceptance markers，只有本 crate 的独立 ELF 同时含 build identity/授权 marker。六类operation均已有独立证据；首个`install_prepared`已闭合checkpoint与exact resume，第二个`install_artifacts_staged`已闭合typed guest合同和checkpoint一次性控制。其余case的精确进度、冻结资产、停止线与下一授权只以[`docs/status/current.md`](../../docs/status/current.md)和[L6 runbook](../../docs/runbooks/linux-l6-package-matrix.md)为准。
