...
发布载体入口位于 `scripts/`：`build-macos-release-installer.sh` 冻结 strict ad-hoc Installer，`build-macos-release-dmg.sh` 生成并复验未公证 APFS/UDZO DMG 与 `CommunityReleaseEvidence.json`。`notarize-macos-release-dmg.sh` 在 `community-adhoc-v1` 稳定失败关闭。证据编码和漂移拒绝集中在 `scripts/macos-product/community_release.py`，不进入 Installer runtime，也不读取用户数据。
...
M4-P02 的 manifest 绑定只解决“执行哪一代、哪一端产品代码”的内容确定性。社区载体还须在 M4-P03 证明产品根来源、strict ad-hoc 内部一致性、DMG SHA-256 和固定安装位置，不能把调用方传入的任意目录直接当作可信产品；它不提供 Apple 发布者认证。

`qualification-harness` Cargo feature 仅供隔离产品协调与安装恢复门禁。它要求 canonical temp 根下的固定 marker 与 `0700` 合成 user home；升级 host 只对子进程设置 `CFFIXED_USER_HOME`，普通 `load` 始终清除该变量。安装 adapter 还要求 payload/home 都是该根内无 symlink、同 owner、精确 mode 的后代。生产与资格均严格解析 ad-hoc identity，但生产 requirement 必须来自 sealed release identity；资格场景使用真实 manifest-bound helper/bundle，故障只在 port 结果边界注入。

## InstallAdapter

adapter 组合 `ime-product-install`，但不接受自定义最终路径、bundle 名或数据路径。构造时要求 authoritative current-user home、uid、InstallPayload 根，以及 Manager/InputMethod 各自有界、排序、无重复的 ad-hoc designated requirement 集合。它逐字节绑定 committed install layout，严格复验 payload format v2、target 与全部 `UpgradeSources` 的 ProductManifest v3、完整 bundle tree、许可证、release 顺序、code identity 和 component-to-target 映射。
