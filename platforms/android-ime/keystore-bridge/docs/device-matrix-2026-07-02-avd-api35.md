# Android Keystore Device Matrix - 2026-07-02 AVD API 35
...
- 执行者：Codex；开发者已启动 Android Studio 和 Pixel 9 Pro API 35 AVD，并要求执行诊断。
- 日期（Asia/Shanghai）：2026-07-02
- 已获批准触碰测试设备 Android Keystore：是
- 测试分支 / commit：`dev` @ `c9d297b`，加本次未提交 diagnostics harness / 文档记录。
- 测试命令：
...
- AVD / physical device：Pixel 9 Pro API 35 AVD
...
- `Signature.getInstance("Ed25519")`：success；algorithm `Ed25519`；provider `AndroidKeyStoreBCWorkaround`
...
- 阻塞原因：Pixel 9 Pro API 35 AVD 上，`Signature.getInstance("Ed25519")` 和 `KeyPairGenerator.getInstance("Ed25519", "AndroidKeyStore")` 均能取得对象，但 `AndroidKeyStore` 实际生成的 key pair 暴露为 `EC`，直接 Ed25519 签名失败，bridge 因此正确收敛为 `unsupported_signature_algorithm`。
- 后续动作：保持 `android-keystore-v1` production status 门禁关闭；继续用同一 diagnostics harness 扩展真实设备 / API / provider 矩阵。若考虑 P-256 或其他算法，必须先补签名算法 ADR、迁移计划和 Go / Rust verifier 变更。
