...
def expected_manifest(
    data_dir: Path, license_file: Path, schema: str, deploy_on_start: bool
) -> dict[str, Any]:
    ...
    if not license_file.is_file() or license_file.stat().st_size == 0:
        raise SystemExit(f"Rime data license is missing or empty: {license_file}")
    for required in ("default.yaml", f"{schema}.schema.yaml"):
    ...
    return {
        ...
        "files": data_hashes(data_dir),
    }


def create_manifest(args: argparse.Namespace) -> int:
    manifest = expected_manifest(
        args.data_dir, args.license, args.schema, args.deploy_on_start
    )
