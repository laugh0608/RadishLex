...
class NativeManifestTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.data_dir / "SourceManifest.json").write_text("{}\n", encoding="utf-8")
        licenses = self.data_dir / "Licenses/component"
    ...
    def test_symlink_is_rejected(self) -> None:
        (self.data_dir / "linked-license").symlink_to(
            self.data_dir / "Licenses/component/LICENSE"
        )
    ...
    def test_missing_component_license_is_rejected(self) -> None:
        (self.data_dir / "Licenses/component/LICENSE").unlink()
        with self.assertRaisesRegex(SystemExit, "component LICENSE"):
            native_manifest.create_manifest(self.arguments())
